...
namespace ModernApplicationFramework.Core.TrinetCoreNtfs
{
    ...
    public static class FileSystem
    {
		...
		public static AlternateDataStreamInfo GetAlternateDataStream(this FileSystemInfo file, string streamName, FileMode mode)
        {
            ...
            {
                throw new IOException($"The specified alternate data stream '{streamName}' does not exist on file '{file.Name}'.");
            }
            if (exists && FileMode.CreateNew == mode)
            {
                throw new IOException($"The specified alternate data stream '{streamName}' already exists on file '{file.Name}'.");
            }
