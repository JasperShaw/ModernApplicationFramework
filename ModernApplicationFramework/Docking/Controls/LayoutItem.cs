	...
	public abstract class LayoutItem : FrameworkElement
	{
		...
		private bool CanExecuteCloseAllButThisCommand(object parameter)
		{
			...
			return LayoutElement.Root.Manager.Layout.
				...
				.Any(d => !Equals(d, LayoutElement) && (d.Parent is LayoutDocumentPane || d.Parent is LayoutDocumentFloatingWindow));
		}
...
#if DEBUG
			if (LayoutElement != null)
				System.Diagnostics.Trace.WriteLine(string.Format("CanExecuteCloseCommand({0}) = {1}", LayoutElement.Title,
