            ...
            if (_handlingMove)
            {
                using (_factory.PerformanceBlockMarker.CreateBlock("VsTextEditor.Scroll.MouseDrag"))
                {
                    ...
                    else
                    {
                        ...
                        if (textLine != null)
                        {
                            if (_wpfTextView.ViewportLeft != 0.0 && !_wpfTextView.Selection.IsEmpty && _wpfTextView.Selection.Mode == TextSelectionMode.Stream && !_wpfTextView.Options.IsVirtualSpaceEnabled() && _wpfTextView.Options.GetOptionValue<bool>(DirectionalSelectionEnabled.DirectionalSelectionEnabledId))
                            {
                                var textViewLines = _wpfTextView.TextViewLines;
