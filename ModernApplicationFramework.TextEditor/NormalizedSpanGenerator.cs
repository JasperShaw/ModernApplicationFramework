...
namespace ModernApplicationFramework.TextEditor
{
    internal static class NormalizedSpanGenerator
    {
        ...
        public static IList<NormalizedSpan> Create(ITextAndAdornmentCollection elements, ITextSnapshot sourceSnapshot, IAccurateClassifier classifier, IClassificationFormatMap classificationFormatMap, CancellationToken? cancel = null)
        {
            ...
            foreach (var element1 in elements)
            {
                ...
                if (element1.ShouldRenderText)
                {
                    ...
                    foreach (var classifiedRun in classifiedRuns)
                    {
                        ...
                        if (classifiedRun.ContainsBidi)
                        {
                            ...
                            var normalizedSpan = new NormalizedSpan(formattingModifier, (PositionAffinity)num, bufferSpan, ref local);
                        ...
                        if (classifiedRun.ContainsBidi)
                        {
                            ...
                            ref var local = ref offset;
                            var normalizedSpan = new NormalizedSpan(endSegment, (PositionAffinity)num, bufferSpan, ref local);
                            normalizedSpanList2.Add(normalizedSpan);
                        }
                    }
                }
                else
                {
                    if (element1 is IAdornmentElement element2)
                        normalizedSpanList1.Add(new NormalizedSpan(element2, classificationFormatMap.DefaultTextProperties, span1, ref offset));
                }
            }
        ...
        private static IList<ClassifiedRun> CreateClassifiedRuns(SnapshotSpan span, IAccurateClassifier classifier, IClassificationFormatMap classificationFormatMap, CancellationToken? cancel)
        {
            ...
            else
            {
                ...
                try
                {
                    classificationSpanList = cancel.HasValue ? classifier.GetAllClassificationSpans(span, cancel.Value) : classifier.GetClassificationSpans(span);
                }
                            ...
                            if (classificationSpan.Span.End > (int)containingLine.End)
                            {
                                ...
                                if (start == num || end2 == num)
                                {
                                    ...
                                    if (start == end2)
                                        ...
                                        classifications[index] = new ClassificationSpan(new SnapshotSpan(span.Snapshot, start, end2 - start), classifications[index].ClassificationType);
                                }
                            }
                            else
                                break;
                ...
                while (end1 - spanStart > NormalizedSpan.MaxTextLineLength)
                {
                    ...
                    if (spanEnd > spanStart + NormalizedSpan.MaxTextLineLength)
                    {
                        ...
                        AddRuns(span.Snapshot, runs, classifications, classificationFormatMap, spanStart, spanEnd, ref startClassificationIndex);
                        spanStart = spanEnd;
                    }
                    else if (flag)
                        ++index1;
                ...
                AddRuns(span.Snapshot, runs, classifications, classificationFormatMap, spanStart, end1, ref startClassificationIndex);
            }
            return runs;
        }

        private static void AddRuns(ITextSnapshot snapshot, IList<ClassifiedRun> runs, IList<ClassificationSpan> classifications, IClassificationFormatMap classificationFormatMap, int spanStart, int spanEnd, ref int startClassificationIndex)
        {
