...
namespace ModernApplicationFramework.TextEditor
{
    public class UnicodeWordExtent
    {
        ...
        public static bool FindCurrentToken(SnapshotPoint currentPosition, out SnapshotSpan span)
        {
            ...
            var ch1 = line[iChar];
            if (IsWordChar(ch1))
                return FindCurrentWordCoords(new SnapshotPoint(currentPosition.Snapshot, containingLine.Start + iChar), out span);
            int num1;
            int index1;
            ...
            else if (char.IsHighSurrogate(ch1) && iChar + 1 < includingLineBreak && char.IsLowSurrogate(line[iChar + 1]))
            {
                num1 = iChar;
                index1 = iChar + 2;
            }
        ...
        public static bool IsPropBreak(char cL, char cR, bool fHanWordBreak)
        {
            ...
            if (unicodeScript1 != unicodeScript2)
            {
                ...
                if (IsPropAlpha(unicodeCategory2) && IsPropAlpha(unicodeCategory1) || IsPropDigit(unicodeCategory2) && IsPropDigit(unicodeCategory1))
                    return true;
            }
            if (unicodeScript1 == UnicodeScript.Cjk || unicodeScript2 == UnicodeScript.Cjk)
            {
                ...
                if (fHanWordBreak && flag3 | flag4 || unicodeScript1 != UnicodeScript.Cjk && unicodeScript2 == UnicodeScript.Cjk)
                    return true;
                var flag5 = IsHiragana(cR);
                if (IsHiragana(cL) && !flag5 || IsKatakana(cL) && !flag5 && !IsKatakana(cR) || flag3 && !(flag4 | flag5))
                    return true;
            }
            return false;
        }
