                ...
                case DropTargetType.DocumentPaneDockInside:
                    {
                        ...
                        if (_tabIndex == -1)
                        {
                            if (DockingManagerPreferences.Instance.DocumentDockPreference ==
                                ...
                                paneModel?.Children.Add(sourceModel);
                        }
                        else
                            paneModel?.Children.Insert(_tabIndex, sourceModel);
                ...
                case DropTargetType.DocumentPaneDockBottom:
...

                    {
                        if (targetModel != null)
                        {
                            ...
                            foreach (
                                var cntToTransfer in
                                    floatingWindow.RootPanel.Descendents().OfType<LayoutAnchorable>().ToArray())
