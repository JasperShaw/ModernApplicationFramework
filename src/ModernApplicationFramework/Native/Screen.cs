    ...
    public static class Screen
    {
        ...
        internal static void FindMaximumSingleMonitorRectangle(RECT windowRect, out RECT screenSubRect, out RECT monitorRect)
        {
            ...
            if (-1 == displayForWindowRect)
                return;
            var display = Displays[displayForWindowRect];
        ...
        {
            var hMonitor = User32.MonitorFromPoint(new Platform.Structs.Point 
            {
                X = (int)point.X,
                Y = (int)point.Y
            }, 2);
        ...
        public static int FindDisplayForWindowRect(Rect windowRect)
        {
            ...
            {
                var rcWork = Displays[index].RcWork;
                User32.IntersectRect(out var lprcDst, ref rcWork, ref lprcSrc2);
            ...
            if (-1 == num1)
            {
                ...
                for (var index = 0; index < Displays.Count; ++index)
                {
                    var num4 = Distance(Displays[index].RcMonitor, lprcSrc2);
                    if (num4 < num3)
                    {
                        num1 = index;
                        num3 = num4;
        ...
        public static int FindDisplayForAbsolutePosition(Point absolutePosition)
        {
            for (var index = 0; index < Displays.Count; ++index)
            {
                var rcMonitor = Displays[index].RcMonitor;
            ...
            for (var index = 0; index < Displays.Count; ++index)
            {
                var num3 = Distance(absolutePosition, Displays[index].RcMonitor);
                if (num3 < num2)
                {
                    num1 = index;
                    num2 = num3;
        ...
        public static Rect AbsoluteRectToRelativeRect(int display, Rect absoluteRect)
        {
            ...
            var rcMonitor = Displays[display].RcMonitor;
        ...
        public static Point RelativePositionToAbsolutePosition(int display, double left, double top)
        {
            ...
            {
                var display1 = Displays[Displays.Count - 1];
        ...
        {
            FindMonitorRectsFromPoint(pt, out var monitorRect, out var workAreaRect);
        ...
        internal static Rect GetOnScreenPosition(Rect floatRect)
        {
            ...
            FindMaximumSingleMonitorRectangle(floatRect, out var rect2, out var rect3);
