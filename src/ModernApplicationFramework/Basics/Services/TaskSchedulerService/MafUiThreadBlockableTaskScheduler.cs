...
namespace ModernApplicationFramework.Basics.Services.TaskSchedulerService
{
    internal abstract class MafUiThreadBlockableTaskScheduler : TaskScheduler, IMafUiThreadBlockableTaskScheduler
    {
        ...
        private readonly Dictionary<Task, TaskCompletionSource<object>> _taskActivationWaiters = new Dictionary<Task, TaskCompletionSource<object>>();
        ...
        {
            _matchingPriorityJoinableTaskFactory = new Lazy<JoinableTaskFactory>(() => new TaskSchedulerJoinableTaskFactory(this));
        }
        ...
        protected override void QueueTask(Task task)
        {
            ...
            factory.StartNew(state => ((TaskCompletionSource<object>)state).TrySetResult(null), completionSource2, none, (TaskCreationOptions)num, scheduler).Forget();
        }
        ...
        private class TaskSchedulerJoinableTaskFactory : JoinableTaskFactory
        {
            ...
            {
                Task.Factory.StartNew(callback.Invoke, state, CancellationToken.None, TaskCreationOptions.None, _taskScheduler).Forget();
            }
        }
    }
}