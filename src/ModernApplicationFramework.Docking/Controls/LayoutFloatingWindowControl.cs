...
{
    public abstract class LayoutFloatingWindowControl : ModernChromeWindow, ILayoutControl, IOverlayWindowHost
    {
        ...
        protected virtual IntPtr FilterMessage(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            switch (msg)
            {
                ...
                case (int)WindowsMessage.WmExitsizemove:
                    ...
                    {
                        var mousePosition = this.TransformToDeviceDpi(NativeMethods.NativeMethods.GetMousePosition());
                ...
                case (int)WindowsMessage.WmSyscommand:
                    var wMaximize = new IntPtr((int)NativeMethods.SystemCommands.ScMaximize);
                    var wRestore = new IntPtr((int)NativeMethods.SystemCommands.ScRestore);
        ...
        {
            if (!NativeMethods.NativeMethods.IsKeyPressed(17) || wParam.ToInt32() != 2)
                return;
            RedockWindow();  
            handled = true;
        }
        ...
        private void TryActivateOwner()
        {
            ...
            if (window != null)
                ...
                NativeMethods.NativeMethods.SetActiveWindow(windowInteropHelper.Owner);
        }
        ...
        private void OnActivated(object sender, EventArgs e)
        {
            ...
            var clientArea = NativeMethods.NativeMethods.GetClientRect(windowHandle);
            var windowArea = NativeMethods.NativeMethods.GetWindowRect(windowHandle);
