...
namespace ModernApplicationFramework.Text.Data.Differencing
{
    public class Difference
    {
        ...
        {
            if (obj is Difference difference && Equals(DifferenceType, difference.DifferenceType) &&
                Equals(Before, difference.Before) && Equals(After, difference.After) && Equals(Left, difference.Left))
        ...
        {
            return (Before.GetHashCode() << 24) ^ (After.GetHashCode() << 16) ^ (Left.GetHashCode() << 8) ^
                   Right.GetHashCode();
        }
        ...
        public override string ToString()
        {
            return string.Format(CultureInfo.CurrentCulture.NumberFormat,
                "(Difference; Type: {0}, Left count: {1}, Right count: {2}", DifferenceType, Left.Length, Right.Length);
        }
    }
}