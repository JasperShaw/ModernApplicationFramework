...
namespace ModernApplicationFramework.Text.Utilities
{
    public sealed class TrackingSpanTree<T>
    {
        ...
        private static IEnumerable<TrackingSpanNode<T>> FindNodes(NormalizedSnapshotSpanCollection spans, TrackingSpanNode<T> root, bool recurse = true, bool contained = false)
        {
        ...
        private static TrackingSpanNode<T> TryAddNodeToRoot(TrackingSpanNode<T> newNode, SnapshotSpan span, TrackingSpanNode<T> root)
        {
            ...
            if (indexForAdd1.Type == FindResultType.Inner || indexForAdd2.Type == FindResultType.Inner)
            {
                ...
                if (indexForAdd1.Index != indexForAdd2.Index && children[indexForAdd2.Index].TrackingSpan.GetSpan(span.Snapshot).Contains(span))
        ...
        private static FindResult FindChild(SnapshotPoint point, List<TrackingSpanNode<T>> nodes, bool left, int lo = -1, int hi = -1)
        {
            if (nodes.Count == 0)
        ...
        private static FindResult FindIndexForAdd(SnapshotPoint point, List<TrackingSpanNode<T>> nodes, bool left, int lo = -1, int hi = -1)
        {
            var snapshot = point.Snapshot;
            var position = point.Position;
            if (lo == -1)
