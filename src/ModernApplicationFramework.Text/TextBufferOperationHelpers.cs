...
namespace ModernApplicationFramework.Text
{
    public static class TextBufferOperationHelpers
    {
        ...
        public static bool TryInsertFinalNewLine(ITextBuffer buffer, IEditorOptions editorOptions)
        {
            ...
            if ((!editorOptions.IsOptionDefined(DefaultOptions.TrimTrailingWhiteSpaceOptionId, true) ? 0 : (!editorOptions.GetOptionValue(DefaultOptions.TrimTrailingWhiteSpaceOptionId) ? 1 : 0)) == 0 && !HasAnyNonWhitespaceCharacters(lineFromLineNumber))
            {
