...
{
    internal class TextMarkerAdapter : ITextMarker, IVsTextLineMarker, IVsTextStreamMarker, IDisposable
    {
        private readonly MarkerManager _manager;
        private uint _clientData;
        private bool _notifyClientOnChanges;
        private ITextMarkerClientEx _clientEx;
        private IReadOnlyRegion _readOnlyRegion;
        private const int ReadOnlyMarkerType = 1;

        internal ITextMarkerClient Client { get; private set; }
    ...
    {
        int OnMarkerTextChanged(ITextMarker pMarker);
    }
