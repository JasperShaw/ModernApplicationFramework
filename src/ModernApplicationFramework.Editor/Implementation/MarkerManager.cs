...
namespace ModernApplicationFramework.Editor.Implementation
{
    internal sealed class MarkerManager
    {
        ...
        public static bool CanAffectClassification(TextMarkerAdapter marker)
        {
        ...
        public static bool CanAffectGlyphMargin(TextMarkerAdapter marker)
        {
        ...
        public MarkerChangedMask GetMarkerChangeMask(TextMarkerAdapter marker)
        {
        ...
        public IList<TextMarkerAdapter> InternalGetMarkers(int markerType, VirtualSnapshotSpan virtualSpan, Enummarkerflags enumMarkerFlags)
        {
            var textMarkerAdapterList = new List<TextMarkerAdapter>();
            lock (_markerStore)
            {
            ...
            if ((enumMarkerFlags & Enummarkerflags.EmSortpriority) != Enummarkerflags.EmDefault)
                textMarkerAdapterList.Sort((enumMarkerFlags & Enummarkerflags.EmSortdescending) != Enummarkerflags.EmDefault ? (a, b) => b.Priority - a.Priority : (Comparison<TextMarkerAdapter>)((a, b) => a.Priority - b.Priority));
        ...
        public bool AddMarker(TextMarkerAdapter marker)
        {
        ...
        public bool RemoveMarker(TextMarkerAdapter marker)
        {
            return RemoveMarker(marker, false);
        }

        public bool RemoveMarker(TextMarkerAdapter marker, bool markerSpanDeleted)
        {
        ...
        {
            foreach (var textMarkerAdapter in new List<TextMarkerAdapter>(_markerStore.AllMarkers))
        ...
        {
            var tupleList = new List<Tuple<Span, TextMarkerAdapter>>();
            lock (_markerStore)
            {
                tupleList.AddRange(from textMarkerAdapter in _markerStore.VisibleMarkersWithColor
                    ...
                    select new Tuple<Span, TextMarkerAdapter>(nullable2.Value, textMarkerAdapter));
            }
        ...
        public static IList<Tuple<Span, TextMarkerAdapter>> GetPrioritizedMarkerSpans(IList<Tuple<Span, TextMarkerAdapter>> unsortedMarkers)
        {
            var tupleList = (IList<Tuple<Span, TextMarkerAdapter>>)new List<Tuple<Span, TextMarkerAdapter>>();
            if (unsortedMarkers.Count > 0)
            {
                ...
                if (markerEdgeList1.Count > 0)
                {
                    ...
                    var textMarkerAdapterList = new List<TextMarkerAdapter>();
                    var textMarkerAdapter = (TextMarkerAdapter)null;
                        ...
                        if (markerEdge.IsStart)
                        {
                            if (markerEdge.Position >= start)
                            {
                                ...
                                if (markerEdge.Marker.Priority > textMarkerAdapter.Priority)
                                {
                                    if (markerEdge.Position > start)
                                    {
                                        tupleList.Add(new Tuple<Span, TextMarkerAdapter>(Span.FromBounds(start, markerEdge.Position), textMarkerAdapter));
        ...
        {
            private readonly HashSet<TextMarkerAdapter> _invisibleMarkers = new HashSet<TextMarkerAdapter>();
            private readonly HashSet<TextMarkerAdapter> _visibleMarkersNoColor = new HashSet<TextMarkerAdapter>();
            private readonly HashSet<TextMarkerAdapter> _visibleMarkersWithColor = new HashSet<TextMarkerAdapter>();
            ...
            public void MarkerTypeChanged(TextMarkerAdapter marker, MarkerType oldType, MarkerType newType, uint oldVisualStyle, uint newVisualStyle)
            {
            ...
            public IEnumerable<TextMarkerAdapter> AllMarkers
            {
                get
                {
            ...
            public List<TextMarkerAdapter> AllMarkersCopy
            {
                get
                {
                    lock (this)
                    {
                        var textMarkerAdapterList = new List<TextMarkerAdapter>(_visibleMarkersNoColor.Count + _visibleMarkersWithColor.Count + _invisibleMarkers.Count);
            ...
            public IEnumerable<TextMarkerAdapter> AllMarkersOfType(int type)
            {
                if (type == 0)
                {
                    foreach (var invisibleMarker in _invisibleMarkers)
            ...
            public IEnumerable<TextMarkerAdapter> VisibleMarkers
            {
                get
                {
                    foreach (var textMarkerAdapter in _visibleMarkersNoColor)
            ...
            public IEnumerable<TextMarkerAdapter> VisibleMarkersWithColor
            {
                get
                {
                    foreach (var textMarkerAdapter in _visibleMarkersWithColor)
            ...
            public IEnumerable<TextMarkerAdapter> VisibleMarkersNoColor
            {
                get
                {
                    foreach (var textMarkerAdapter in _visibleMarkersNoColor)
            ...
            public IEnumerable<TextMarkerAdapter> InvisibleMarkers => _invisibleMarkers;
        }
        ...
        {
            public readonly TextMarkerAdapter Marker;
            public readonly int Position;
            public readonly bool IsStart;

            public MarkerEdge(TextMarkerAdapter marker, int position, bool isStart)
            {
                Marker = marker;
                Position = position;
                IsStart = isStart;
