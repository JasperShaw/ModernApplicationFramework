...
namespace ModernApplicationFramework.Modules.Editor.AdornmentLibrary.TextMarkers
{
    internal class TextMarkerVisualManager
    {
        ...
        internal TextMarkerVisualManager(ITextView textView,
            IViewTagAggregatorFactoryService tagAggregatorFactoryService,
            IEditorFormatMapService editorFormatMapService)
        {
            _textView = textView;
            TagAggregator =
                tagAggregatorFactoryService.CreateTagAggregator<ITextMarkerTag>(textView, (TagAggregatorOptions) 2);
        ...
        private void AddAdornment(IMappingTagSpan<ITextMarkerTag> tag, Dictionary<ITextViewLine, Span> lineCache,
            NormalizedSnapshotSpanCollection applicabilitySpans, SnapshotSpan formattedSpan)
        {
            foreach (var span in tag.Span.GetSpans(_textView.TextSnapshot))
            {
                var nullable = formattedSpan.Intersection(span);
                if (nullable.HasValue &&
                    (applicabilitySpans == null || applicabilitySpans.OverlapsWith(nullable.Value)))
                    AddAdornment(nullable.Value, span, tag, lineCache);
            }
        }

        private void AddAdornment(SnapshotSpan span, SnapshotSpan sourceSpan, IMappingTagSpan<ITextMarkerTag> tag,
            Dictionary<ITextViewLine, Span> lineCache)
        {
            var containingBufferPosition = _textView.TextViewLines.GetTextViewLineContainingBufferPosition(span.Start);
            var useTextBounds = containingBufferPosition != null &&
                                span.End.Position <= containingBufferPosition.EndIncludingLineBreak;
            if (useTextBounds)
            {
                if (!lineCache.TryGetValue(containingBufferPosition, out var span1))
                {
                    span1 = Span.FromBounds(GetBufferPositionFromXCoordinate(containingBufferPosition, Left - 1000.0),
                        GetBufferPositionFromXCoordinate(containingBufferPosition, Right + 1000.0));
            ...
            var rectanglesFromBounds = Markers.GetRectanglesFromBounds(
                _textView.TextViewLines.GetNormalizedTextBounds(span),
                useTextBounds ? Markers.SingleLinePadding : Markers.MultiLinePadding, Left, Right, useTextBounds);
            if (rectanglesFromBounds.Count <= 0)
                return;
            var adornmentForGeometry =
                GetAdornmentForGeometry(Markers.GetMarkerGeometryFromRectangles(rectanglesFromBounds), tag);
            if (Panel.GetZIndex(adornmentForGeometry) >= 0)
                _textMarkerLayer.AddAdornment(AdornmentPositioningBehavior.TextRelative, sourceSpan, tag,
                    adornmentForGeometry, null);
            else
                _negativeTextMarkerLayer.AddAdornment(AdornmentPositioningBehavior.TextRelative, sourceSpan, tag,
                    adornmentForGeometry, null);
        }
        ...
        private UIElement GetAdornmentForGeometry(Geometry geometry, IMappingTagSpan<ITextMarkerTag> tag)
        {
            ...
            if (fillBrush == null)
            {
                fillBrush = new SolidColorBrush(Color.FromArgb(32, Colors.DarkGray.R, Colors.DarkGray.G,
                    Colors.DarkGray.B));
            }
        ...
        private void OnBatchedTagsChanged(object sender, BatchedTagsChangedEventArgs e)
        {
            ...
            AddAdornments(new NormalizedSnapshotSpanCollection(snapshotSpanList), _textView.ViewportLeft,
                _textView.ViewportRight);
        }
