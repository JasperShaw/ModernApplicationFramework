...
namespace ModernApplicationFramework.Modules.Editor.Implementation
{
    ...
    internal sealed class TextEditorFactoryService : ITextEditorFactoryService, IPartImportsSatisfiedNotification
    {
        ...
        [ImportMany] private List<Lazy<AdornmentLayerDefinition, IAdornmentLayersMetadata>> _viewLayerDefinitions;

        public event EventHandler<TextViewCreatedEventArgs> TextViewCreated;

        public ITextViewRoleSet AllPredefinedRoles => CreateTextViewRoleSet("ANALYZABLE", "DEBUGGABLE", "DOCUMENT",
            "EDITABLE", "INTERACTIVE", "STRUCTURED", "ZOOMABLE", "PRIMARYDOCUMENT");
        ...
        [Import] internal IFormattedTextSourceFactoryService FormattedTextSourceFactoryService { get; set; }

        [Import] internal GuardedOperations GuardedOperations { get; set; }

        [ImportMany]
        internal List<Lazy<ILineTransformSourceProvider, IContentTypeAndTextViewRoleMetadata>>
            LineTransformSourceProviders { get; set; }

        [Import] internal TextViewMarginState MarginState { get; set; }

        [ImportMany] internal List<Lazy<IObjectTracker>> ObjectTrackers { get; set; }

        [Import(AllowDefault = true)] internal IOutliningManagerService OutliningManagerService { get; set; }

        [Import] internal PerformanceBlockMarker PerformanceBlockMarker { get; set; }
        ...
        internal List<Lazy<ITextViewConnectionListener, IContentTypeAndTextViewRoleMetadata>>
            TextViewConnectionListeners { get; set; }

        [ImportMany(typeof(ITextViewCreationListener))]
        internal List<Lazy<ITextViewCreationListener, IDeferrableContentTypeAndTextViewRoleMetadata>>
            TextViewCreationListeners { get; set; }

        [ImportMany]
        internal List<Lazy<ITextViewModelProvider, IContentTypeAndTextViewRoleMetadata>> TextViewModelProviders
        {
            get;
            set;
        }
        ...
        public ITextView CreateTextView(ITextBuffer textBuffer, ITextViewRoleSet roles)
        {
            ...
            return CreateTextView(new VacuousTextDataModel(textBuffer), roles,
        ...
        private TextView CreateAndTrackTextView(ITextDataModel dataModel, ITextViewRoleSet roles,
        ...
        private void InitializeTextView(ITextView view)
        {
            ...
            foreach (var objectTracker in ObjectTrackers)
