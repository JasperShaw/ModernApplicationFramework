    ...
    internal sealed class DefaultViewScroller : IViewScroller
    {
                ...
                case ScrollDirection.Down:
                    ...
                    if (direction == ScrollDirection.Up)
                    {
                        if (firstVisibleLine.Start > 0)
                        {
                            var containingBufferPosition =
                                textViewLines.GetTextViewLineContainingBufferPosition(firstVisibleLine.Start - 1);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            var containingBufferPosition =
                                textViewLines.GetTextViewLineContainingBufferPosition(firstVisibleLine
                ...
                case ScrollDirection.Down:
                    ...
                    if (textViewLines1 != null)
                    {
                            ...
                            if (firstVisibleLine2.VisibilityState != VisibilityState.FullyVisible)
                            {
                                ...
                                if (textViewLine.VisibilityState == VisibilityState.FullyVisible)
                                    _textView.DisplayTextLineContainingBufferPosition(
                                        firstVisibleLine2.EndIncludingLineBreak, 0.0, ViewRelativePosition.Top);
                                else if (textViewLine.Bottom - _textView.ViewportBottom >
                                         firstVisibleLine2.Bottom - _textView.ViewportTop)
                                    _textView.DisplayTextLineContainingBufferPosition(
                                        firstVisibleLine2.EndIncludingLineBreak, 0.0, ViewRelativePosition.Top);
                            }
                        }
                        else
                        {
                            ...
                            {
                                start = textViewLines1
                                    .GetTextViewLineContainingBufferPosition(lastVisibleLine.EndIncludingLineBreak)
                                    .Start;
                            }
        ...
        public void ScrollViewportVerticallyByPixels(double pixelsToScroll)
        {
            ...
            if (pixelsToScroll >= 0.0)
            {
                ...
                _textView.DisplayTextLineContainingBufferPosition(textViewLine.Start,
                    textViewLine.Top - _textView.ViewportTop + pixelsToScroll, ViewRelativePosition.Top);
            }
            else
            {
        ...
        private void EnsureSpanFormatted(SnapshotSpan span, EnsureSpanVisibleOptions options)
        {
            ...
            do
            {
                ...
                flag1 = flag1 && (containingBufferPosition.VisibilityState == VisibilityState.Unattached ||
                                  containingBufferPosition.VisibilityState == VisibilityState.Hidden);
                bufferPosition = containingBufferPosition.EndIncludingLineBreak;
            } while (bufferPosition.Position < span.End);

            if ((options & EnsureSpanVisibleOptions.AlwaysCenter) != EnsureSpanVisibleOptions.None || flag1 &&
                (options & EnsureSpanVisibleOptions.MinimumScroll) == EnsureSpanVisibleOptions.None)
            {
                _textView.DisplayTextLineContainingBufferPosition(span.Start, (_textView.ViewportHeight - num) * 0.5,
        ...
        private void InnerEnsureSpanVisible(VirtualSnapshotSpan span, EnsureSpanVisibleOptions options)
        {
            if (_textView.IsClosed)
                return;
            if ((options & ~(EnsureSpanVisibleOptions.ShowStart | EnsureSpanVisibleOptions.MinimumScroll |
                             EnsureSpanVisibleOptions.AlwaysCenter)) != EnsureSpanVisibleOptions.None)
            ...
            if ((_textView.Options.GetOptionValue(DefaultTextViewOptions.WordWrapStyleId) & WordWrapStyles.WordWrap) ==
            ...
            var leading1 = textView1.GetTextViewLineContainingBufferPosition(position1)
                .GetExtendedCharacterBounds(span.Start).Leading;
            if (leading1 < num2)
                num2 = leading1;
            if (leading1 > num3)
                num3 = leading1;
            ...
            if (virtualSnapshotPoint.IsInVirtualSpace)
            {
                ...
                var leading2 = textView2.GetTextViewLineContainingBufferPosition(position2)
                    .GetExtendedCharacterBounds(span.End).Leading;
                if (leading2 < num2)
                    num2 = leading2;
                if (leading2 > num3)
                    num3 = leading2;
            ...
            {
                var bufferPosition = (options & EnsureSpanVisibleOptions.ShowStart) != EnsureSpanVisibleOptions.None
                    ? span.Start
                    : span.End;
                var leading2 = _textView.GetTextViewLineContainingBufferPosition(bufferPosition.Position)
                    .GetExtendedCharacterBounds(bufferPosition).Leading;
