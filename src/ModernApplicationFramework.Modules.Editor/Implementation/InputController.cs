...
namespace ModernApplicationFramework.Modules.Editor.Implementation
{
    internal sealed class InputController
    {
        ...
        internal InputController(ITextView theTextView, InputControllerViewCreationListener factory)
        {
            ...
            _masterMouseProcessor = new MasterMouseProcessor(_textView,
                _state.EditorOperationsFactoryService.GetEditorOperations(_textView),
                mouseProcessorList ?? new List<IMouseProcessor>(0), _factory);
            _keyProcessorDispatcher = new KeyProcessorDispatcher(_textView,
                keyProcessorList ?? new List<KeyProcessor>(0), _factory.GuardedOperations);
        ...
        private List<KeyProcessor> UpdateKeyProcessors(IEnumerable<IContentType> bufferContentTypes)
        {
            ...
            for (var index = 0; index < _state.OrderedKeyProcessorProviders.Count; ++index)
            {
                ...
                if (_textView.Roles.ContainsAny(processorProvider.Metadata.TextViewRoles) &&
                    ExtensionSelector.ContentTypeMatch(bufferContentTypes, processorProvider.Metadata.ContentTypes))
                {
                    if (ActiveKeyProcessors[index] != null)
                        continue;
                    ActiveKeyProcessors[index] = _factory.GuardedOperations.InstantiateExtension(processorProvider,
                        processorProvider, p => p.GetAssociatedProcessor(_textView));
                    flag = true;
                }
        ...
        private List<IMouseProcessor> UpdateMouseHandlers(IEnumerable<IContentType> bufferContentTypes)
        {
            ...
            for (var index = 0; index < _state.OrderedMouseProcessorProviders.Count; ++index)
            {
                    ...
                    ExtensionSelector.ContentTypeMatch(bufferContentTypes, processorProvider.Metadata.ContentTypes))
                {
                    ...
                    {
                        ActiveMouseProcessors[index] =
                            _factory.GuardedOperations.InstantiateExtension(processorProvider, processorProvider,
                                p => p.GetAssociatedProcessor(_textView));
                        flag = true;
                    }
                }
