...
namespace ModernApplicationFramework.Modules.Editor.Implementation
{
    internal sealed class MasterMouseProcessor : WpfMouseProcessor
    {
        ...
        private readonly double _deltaThreshold = 0.5;
        ...
        public MasterMouseProcessor(ITextView wpfTextView, IEditorOperations editorOperations,
            IList<IMouseProcessor> mouseProcessors, InputControllerViewCreationListener factory)
            ...
            if (_handlingMove)
                using (_factory.PerformanceBlockMarker.CreateBlock("VsTextEditor.Scroll.MouseDrag"))
                {
                    ...
                    else
                    {
                        ...
                        if (textLine != null)
                        {
                            if (_wpfTextView.ViewportLeft != 0.0 && !_wpfTextView.Selection.IsEmpty &&
                                _wpfTextView.Selection.Mode == TextSelectionMode.Stream &&
                                !_wpfTextView.Options.IsVirtualSpaceEnabled() &&
                                _wpfTextView.Options.GetOptionValue(DirectionalSelectionEnabled
                                    .DirectionalSelectionEnabledId))
                            {
                                ...
                                var containingBufferPosition =
                                    textViewLines.GetTextViewLineContainingBufferPosition(position1);
                                if (containingBufferPosition != null && containingBufferPosition != textLine)
                                {
                                    ...
                                    {
                                        var characterBounds =
                                            containingBufferPosition.GetCharacterBounds(_wpfTextView.Selection
                                                .AnchorPoint);
        ...
        {
            if (_touchStartedZoom) _touchStartedZoom = false;
        ...
        protected override void DefaultManipulationDeltaHandler(object sender, ManipulationDeltaEventArgs e)
        {
            ...
            else if (e.DeltaManipulation.Translation.X != 0.0 || e.DeltaManipulation.Translation.Y != 0.0)
            {
                ...
                else if (_currentTouchScrollMode == TouchScrollMode.Horizontal && num1 != 0.0 &&
                         num2 - num1 > _deltaThreshold)
                {
                    _currentTouchScrollMode = TouchScrollMode.Diagonal;
                }
                ...
                if (_currentTouchScrollMode == TouchScrollMode.Vertical ||
                    _currentTouchScrollMode == TouchScrollMode.Diagonal)
                {
                    _wpfTextView.ViewScroller.ScrollViewportVerticallyByPixels(
                        e.DeltaManipulation.Translation.Y * num3);
                    _touchStartedScroll = true;
                }

                if (_currentTouchScrollMode == TouchScrollMode.Horizontal ||
                    _currentTouchScrollMode == TouchScrollMode.Diagonal)
                {
                    _wpfTextView.ViewScroller.ScrollViewportHorizontallyByPixels(
                        e.DeltaManipulation.Translation.X * -1.0 * num3);
                    _touchStartedScroll = true;
                }
            }
        ...
        protected override void DefaultManipulationInertiaStartingHandler(object sender,
        ...
        {
            e.Handled = HandleMouseLeftButtonDown((Keyboard.Modifiers & ModifierKeys.Shift) == ModifierKeys.Shift,
                (Keyboard.Modifiers & ModifierKeys.Alt) == ModifierKeys.Alt, e.GetPosition(_wpfTextView.VisualElement));
        }
        ...
        {
            e.Handled = HandleMouseMove(e.LeftButton, (Keyboard.Modifiers & ModifierKeys.Alt) == ModifierKeys.Alt,
                e.GetPosition(_wpfTextView.VisualElement));
        }
        ...
        private void HandleScrollTimerTick(object timer, ScrollDirection direction, double deltaX)
        {
            ...
            {
                if (_wpfTextView.TextViewLines.LastVisibleLine.EndIncludingLineBreak !=
                    _wpfTextView.TextSnapshot.Length)
                    return;
                StopScrolling();
            }
        }
        ...
        private void ScrollAndExtendSelection(ScrollDirection direction, double deltaX)
        {
            ...
            if (textViewLines == null)
                return;
            _editorOperations.MoveCaret(
                direction == ScrollDirection.Up ? textViewLines.FirstVisibleLine : textViewLines.LastVisibleLine,
                deltaX, true);
            _wpfTextView.Caret.EnsureVisible();
        }
