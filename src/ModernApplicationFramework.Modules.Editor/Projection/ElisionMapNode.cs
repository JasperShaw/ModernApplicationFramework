...
namespace ModernApplicationFramework.Modules.Editor.Projection
{
    internal class ElisionMapNode
    {
        ...
        public ElisionMapNode(int exposedSize, int sourceSize, int exposedLineBreakCount, int sourceLineBreakCount,
            bool leftmostElision)
        {
        ...
        public ElisionMapNode(int exposedSize, int sourceSize, int exposedLineBreakCount, int sourceLineBreakCount,
            ElisionMapNode left, ElisionMapNode right, bool leftmostElision)
        {
            _exposedSize = exposedSize;
            _sourceSize = sourceSize;
            _exposedLineBreakCount = exposedLineBreakCount;
            ...
            TotalExposedLineBreakCount = LeftTotalExposedLineBreakCount() + exposedLineBreakCount +
                                         RightTotalExposedLineBreakCount();
            TotalSourceLineBreakCount = LeftTotalSourceLineBreakCount() + sourceLineBreakCount +
                                        RightTotalSourceLineBreakCount();
            _leftmostElision = leftmostElision;
        }
        ...
        public ProjectionLineInfo GetLineFromPosition(ITextSnapshot sourceSnapshot, int exposedPosition,
            int sourcePrefixLineBreakCount, int hiddenPrefixLineBreakCount, int sourcePrefixSize, int exposedPrefixSize,
            int level)
        {
            ...
            do
            {
                ...
                {
                    var lineFromPosition = _left.GetLineFromPosition(sourceSnapshot, exposedPosition1,
                        sourcePrefixLineBreakCount, hiddenPrefixLineBreakCount, sourcePrefixSize, exposedPrefixSize,
                        level + 1);
                ...
                else if (exposedPosition1 < span.End || _right == null)
                {
                    ...
                    var num1 = lineFromPosition.LineNumber -
                    ...
                    if (calculationState1 == ProjectionLineCalculationState.Prepend &&
                        num1 < projectionLineInfo.LineNumber)
                    {
                        projectionLineInfo.StartComplete = true;
                    }
                    else if (calculationState1 == ProjectionLineCalculationState.Primary ||
                ...
                {
                    var lineFromPosition = _right.GetLineFromPosition(sourceSnapshot,
                        exposedPosition1 - (LeftTotalExposedSize() + _exposedSize),
                        sourcePrefixLineBreakCount + LeftTotalSourceLineBreakCount() + _sourceLineBreakCount,
                        hiddenPrefixLineBreakCount + LeftTotalHiddenLineBreakCount() +
                        (_sourceLineBreakCount - _exposedLineBreakCount),
                        sourcePrefixSize + LeftTotalSourceSize() + _sourceSize,
                        exposedPrefixSize + LeftTotalExposedSize() + _exposedSize, level + 1);
                    if (calculationState1 == ProjectionLineCalculationState.Primary)
                    {
        ...
        public int GetLineNumberFromPosition(ITextSnapshot sourceSnapshot, int exposedPosition,
            int hiddenPrefixLineBreakCount, int sourcePrefixSize)
        {
            var span = new Span(LeftTotalExposedSize(), _exposedSize);
            if (exposedPosition < span.Start)
                return _left.GetLineNumberFromPosition(sourceSnapshot, exposedPosition, hiddenPrefixLineBreakCount,
                    sourcePrefixSize);
            if (exposedPosition < span.End || _right == null)
                return sourceSnapshot.GetLineNumberFromPosition(
                           exposedPosition + sourcePrefixSize + LeftTotalSourceSize() - LeftTotalExposedSize()) -
                       (LeftTotalHiddenLineBreakCount() + hiddenPrefixLineBreakCount);
            return _right.GetLineNumberFromPosition(sourceSnapshot, exposedPosition - span.End,
                hiddenPrefixLineBreakCount + LeftTotalHiddenLineBreakCount() +
                (_sourceLineBreakCount - _exposedLineBreakCount),
                sourcePrefixSize + LeftTotalSourceSize() + _sourceSize);
        }

        public void GetSourceSpans(ITextSnapshot sourceSnapshot, ref int rank, ref int sourcePrefixSize,
            int startSpanIndex, int endSpanIndex, IList<SnapshotSpan> result)
        {
            ...
            _right?.GetSourceSpans(sourceSnapshot, ref rank, ref sourcePrefixSize, startSpanIndex, endSpanIndex,
                result);
        }

        public ElisionMapNode IncorporateChange(ITextSnapshot beforeSourceSnapshot, ITextSnapshot afterSourceSnapshot,
            ITextSnapshot beforeElisionSnapshot, int? sourceInsertionPosition, StringRebuilder newText,
            Span? sourceDeletionSpan, int absoluteSourceOldPosition, int absoluteSourceNewPosition,
            int projectedPrefixSize, FrugalList<TextChange> projectedChanges, int incomingAccumulatedDelta,
            ref int outgoingAccumulatedDelta, ref int accumulatedDelete)
        {
            var left = _left;
            var right1 = _right;
            ...
            if (flag1 || nullable1.HasValue)
                left = _left.IncorporateChange(beforeSourceSnapshot, afterSourceSnapshot, beforeElisionSnapshot,
                    sourceInsertionPosition1, newText, span1.Overlap(sourceDeletionSpan), absoluteSourceOldPosition,
                    absoluteSourceNewPosition, projectedPrefixSize, projectedChanges, incomingAccumulatedDelta,
            ...
            if (sourceInsertionPosition.HasValue)
            {
                if (_leftmostElision)
                {
                    if (sourceInsertionPosition.Value <= _sourceSize)
                    {
                        ...
                        ComputeIncrementalLineCountForHiddenInsertion(afterSourceSnapshot, absoluteSourceNewPosition,
                            newText, out var incrementalLineCount);
                ...
                else if (num1 <= sourceInsertionPosition.Value && sourceInsertionPosition.Value <= num1 + _exposedSize)
                {
                    ...
                    sourceSize += newText.Length;
                    var oldPosition = projectedPrefixSize + sourceInsertionPosition.Value - LeftTotalHiddenSize() -
                                      incomingAccumulatedDelta;
                    var num2 = 0;
                    if (nullable2.HasValue)
                        num2 = nullable2.Value.Length;
                    ComputeIncrementalLineCountForExposedInsertion(
                        sourceInsertionPosition.Value > span2.Start
                            ? afterSourceSnapshot[absoluteSourceNewPosition - 1]
                            : new char?(),
                        sourceInsertionPosition.Value + num2 < span2.End
                            ? afterSourceSnapshot[absoluteSourceNewPosition + newText.Length]
                            : new char?(), newText, out var incrementalLineCount, out var boundaryConditions);
                ...
                else if (num1 + _exposedSize < sourceInsertionPosition.Value &&
                         (sourceInsertionPosition.Value < num1 + _sourceSize || _right == null))
                {
            ...
            if (nullable2.HasValue)
            {
                ...
                ComputeIncrementalLineCountForDeletion(beforeSnapshot, deletionSpan, deletedText, out var num5,
                    out var boundaryConditions);
                exposedLineBreakCount += num5;
                sourceLineBreakCount += num5;
                var textChange = new TextChange(num3 - incomingAccumulatedDelta, oldText, StringRebuilder.Empty,
                    boundaryConditions);
                projectedChanges.Add(textChange);
                outgoingAccumulatedDelta += textChange.Delta;
            ...
            if (nullable3.HasValue)
            {
                ...
                ComputeIncrementalLineCountForDeletion(beforeSnapshot, deletionSpan, deletedText, out var num3,
                    out var boundaryConditions);
            ...
            var flag2 = sourceInsertionPosition.HasValue && _right != null &&
                        num1 + _sourceSize <= sourceInsertionPosition.Value;
            if (nullable4.HasValue | flag2)
            {
                ...
                right1 = right2.IncorporateChange(beforeSourceSnapshot1, afterSourceSnapshot1, beforeElisionSnapshot1,
                    sourceInsertionPosition2, newText1, sourceDeletionSpan1, absoluteSourceOldPosition1,
                    absoluteSourceNewPosition1, projectedPrefixSize1, projectedChanges1, incomingAccumulatedDelta1,
                    ref local1, ref local2);
            }

            return new ElisionMapNode(exposedSize, sourceSize, exposedLineBreakCount, sourceLineBreakCount, left,
                right1, leftmostElision);
        }
            ...
            if (sourcePosition < num + _sourceSize)
            {
                if (sourcePosition <= num + _exposedSize && !_leftmostElision)
                    return new SnapshotPoint(snapshot,
                        exposedPrefixLength + LeftTotalExposedSize() + (sourcePosition - num));
                return new SnapshotPoint?();
            }

            if (_right != null)
                return _right.MapFromSourceSnapshot(snapshot, sourcePosition - (num + _sourceSize),
                    exposedPrefixLength + LeftTotalExposedSize() + _exposedSize);
        ...
        public void MapFromSourceSnapshot(Span mapSpan, int exposedPrefixSize, FrugalList<Span> result)
        {
            ...
            if (nullable2.HasValue)
                result.Add(new Span(nullable2.Value.Start + exposedPrefixSize - LeftTotalHiddenSize(),
                    nullable2.Value.Length));
            if (!nullable3.HasValue)
                return;
            _right.MapFromSourceSnapshot(new Span(nullable3.Value.Start - num - _sourceSize, nullable3.Value.Length),
                exposedPrefixSize + LeftTotalExposedSize() + _exposedSize, result);
        }
        ...
        public void MapInsertionPointToSourceSnapshots(IElisionSnapshot elisionSnapshot, int exposedPosition,
            int sourcePrefixLength, FrugalList<SnapshotPoint> points)
        {
            ...
            if ((!_leftmostElision ? 0 : (elisionSnapshot.Length > 0 ? 1 : 0)) == 0 && exposedPosition >= num &&
                exposedPosition <= num + _exposedSize)
                points.Add(new SnapshotPoint(elisionSnapshot.SourceSnapshot,
                    sourcePrefixLength + LeftTotalSourceSize() + (exposedPosition - num)));
            if (_right == null || exposedPosition < num + _exposedSize)
                return;
            _right.MapInsertionPointToSourceSnapshots(elisionSnapshot,
                exposedPosition - (LeftTotalExposedSize() + _exposedSize),
                sourcePrefixLength + LeftTotalSourceSize() + _sourceSize, points);
        }
        ...
        public void MapNullSpanFromSourceSnapshot(Span nullSourceSpan, int exposedPrefixSize, FrugalList<Span> result)
        {
            ...
            if (!span3.IntersectsWith(nullSourceSpan))
                return;
            _right.MapNullSpanFromSourceSnapshot(
                new Span(span3.Intersection(nullSourceSpan).Value.Start - num - _sourceSize, 0),
                exposedPrefixSize + LeftTotalExposedSize() + _exposedSize, result);
        }

        public SnapshotPoint MapToSourceSnapshot(ITextSnapshot sourceSnapshot, int exposedPosition,
            int sourcePrefixLength, PositionAffinity affinity)
        {
            var num = LeftTotalExposedSize();
            if (affinity == PositionAffinity.Predecessor && exposedPosition <= num && _left != null &&
                !_left._leftmostElision || affinity == PositionAffinity.Successor && exposedPosition < num)
                return _left.MapToSourceSnapshot(sourceSnapshot, exposedPosition, sourcePrefixLength, affinity);
            if (affinity == PositionAffinity.Predecessor && exposedPosition <= num + _exposedSize ||
                affinity == PositionAffinity.Successor && exposedPosition < num + _exposedSize)
            ...
            return _right.MapToSourceSnapshot(sourceSnapshot, exposedPosition - (num + _exposedSize),
                sourcePrefixLength + LeftTotalSourceSize() + _sourceSize, affinity);
        }

        public void MapToSourceSnapshots(ITextSnapshot sourceSnapshot, Span mapSpan, int sourcePrefixSize,
            FrugalList<SnapshotSpan> result)
        {
            ...
            if (nullable2.HasValue)
            {
                var span4 = new Span(nullable2.Value.Start + sourcePrefixSize + LeftTotalHiddenSize(),
                    nullable2.Value.Length);
        ...
        {
            return string.Format(CultureInfo.InvariantCulture, "Exp:{0} Src:{1} TExp:{2} Tsrc:{3}", _exposedSize,
                _sourceSize, TotalExposedSize, TotalSourceSize);
        }

        private static void ComputeIncrementalLineCountForDeletion(ITextSnapshot beforeSnapshot, Span deletionSpan,
            StringRebuilder deletedText, out int incrementalLineCount,
            out LineBreakBoundaryConditions boundaryConditions)
        {
            ...
            if (boundaryConditions1 == (LineBreakBoundaryConditions.PrecedingReturn |
                                        LineBreakBoundaryConditions.SucceedingNewline))
                --num;
            incrementalLineCount = num;
            boundaryConditions = boundaryConditions1;
        }

        private static void ComputeIncrementalLineCountForExposedInsertion(char? predecessor, char? successor,
            StringRebuilder insertedText, out int incrementalLineCount,
            out LineBreakBoundaryConditions boundaryConditions)
        {
        ...
        private static void ComputeIncrementalLineCountForHiddenInsertion(ITextSnapshot afterSnapshot, int start,
            StringRebuilder insertedText, out int incrementalLineCount)
        {
            var lineBreakCount = insertedText.LineBreakCount;
            ...
            if (boundaryConditions == (LineBreakBoundaryConditions.PrecedingReturn |
                                       LineBreakBoundaryConditions.SucceedingNewline))
                ++lineBreakCount;
            incrementalLineCount = lineBreakCount;
        }

        private int GetPositionFromLineNumber(ITextSnapshot sourceSnapshot, int relativeExposedLineNumber,
            int sourcePrefixLineBreakCount, int sourcePrefixHiddenSize)
        {
            if (relativeExposedLineNumber < LeftTotalExposedLineBreakCount())
                return _left.GetPositionFromLineNumber(sourceSnapshot, relativeExposedLineNumber,
                    sourcePrefixLineBreakCount, sourcePrefixHiddenSize);
            if (relativeExposedLineNumber >= LeftTotalExposedLineBreakCount() + _exposedLineBreakCount &&
                _right != null)
                return _right.GetPositionFromLineNumber(sourceSnapshot,
                    relativeExposedLineNumber - (LeftTotalExposedLineBreakCount() + _exposedLineBreakCount),
                    sourcePrefixLineBreakCount + LeftTotalSourceLineBreakCount() + _sourceLineBreakCount,
                    sourcePrefixHiddenSize + LeftTotalHiddenSize() + (_sourceSize - _exposedSize));
            var lineNumber = sourcePrefixLineBreakCount + LeftTotalHiddenLineBreakCount() + relativeExposedLineNumber;
            return sourceSnapshot.GetLineFromLineNumber(lineNumber).End - sourcePrefixHiddenSize -
                   LeftTotalHiddenSize();
        }
