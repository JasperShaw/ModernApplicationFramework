...
namespace ModernApplicationFramework.Modules.Editor.Projection
{
    internal class ProjectionSnapshot : BaseProjectionSnapshot
    {
        ...
        public ProjectionSnapshot(ProjectionBuffer projectionBuffer, ITextVersion version, StringRebuilder content,
            IList<SnapshotSpan> sourceSpans)
            : base(version, content)
        {
            ...
            for (var index = 0; index < sourceSpans.Count; ++index)
            {
                ...
                var num = sourceSpan.Snapshot.GetLineNumberFromPosition(sourceSpan.End) -
            ...
            switch (affinity)
            {
                ...
                case PositionAffinity.Successor:
                    ...
                    while (num1 <= num2)
                    {
                        ...
                        {
                            nullable = new SnapshotPoint(this,
                                invertedSourceList[index].ProjectedPosition + sourcePoint.Position - sourceSpan.Start);
        ...
        public override SnapshotPoint MapToSourceSnapshot(int position)
        {
            ...
            return sourceSnapshots[
                _projectionBuffer.Resolver.GetTypicalInsertionPosition(new SnapshotPoint(this, position),
                    sourceSnapshots)];
        }
        ...
        public override SnapshotPoint MapToSourceSnapshot(int position, PositionAffinity affinity)
        {
            ...
            if (affinity < PositionAffinity.Predecessor || affinity > PositionAffinity.Successor)
                throw new ArgumentOutOfRangeException(nameof(affinity));
            var index = affinity == PositionAffinity.Predecessor
                ? FindLowestSpanIndexOfPosition(position)
        ...
        public override ReadOnlyCollection<SnapshotSpan> MapToSourceSnapshotsForRead(Span span)
        {
            ...
            else
            {
                ...
                var length = start.Position + span.Length < (int) sourceSpan1.End
                    ? span.Length
                    : sourceSpan1.End.Position - start;
                ...
                while (length < span.Length)
                {
                    ...
                    else
                    {
                        ...
                        frugalList.Add(new SnapshotSpan(sourceSpan2.Snapshot,
                            new Span(sourceSpan2.Start, span.End - _cumulativeLengths[spanIndexOfPosition])));
                    }
                }
            }
            ...
            ITextBuffer excludedBuffer)
        {
            ...
            while (++spanIndexOfPosition < _sourceSpans.Count && _cumulativeLengths[spanIndexOfPosition] ==
                   _cumulativeLengths[spanIndexOfPosition + 1])
            {
            ...
            ITextBuffer excludedBuffer)
        {
            ...
            var length = start.Position + replacementSpan.Length < (int) sourceSpan1.End
            ...
            while (spanIndexOfPosition1 < spanIndexOfPosition2)
            {
                ...
                var snapshotSpan2 = replacementSpan.End >= _cumulativeLengths[spanIndexOfPosition1 + 1]
                    ? sourceSpan2
                    : new SnapshotSpan(sourceSpan2.Snapshot,
                        new Span(sourceSpan2.Start, replacementSpan.End - _cumulativeLengths[spanIndexOfPosition1]));
        ...
        private static void MapDownToGround(IList<SnapshotSpan> spans,
            Dictionary<ITextSnapshot, List<Span>> groundSourceSpansMap)
        {
        ...
        private string LocalToString()
        {
            ...
            stringBuilder.AppendFormat(CultureInfo.InvariantCulture, "Snapshot {0,10} V{1}\r\n",
                TextUtilities.GetTagOrContentType(_projectionBuffer), Version.VersionNumber);
            var start = 0;
