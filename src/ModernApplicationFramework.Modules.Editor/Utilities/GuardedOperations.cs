...
namespace ModernApplicationFramework.Modules.Editor.Utilities
{
    ...
    {
        [ImportMany] private List<Lazy<IExtensionErrorHandler>> _errorHandlerExports;
        ...
        public IEnumerable<Lazy<TExtensionFactory, TMetadataView>> FindEligibleFactories<TExtensionFactory,
            TMetadataView>(IEnumerable<Lazy<TExtensionFactory, TMetadataView>> lazyFactories,
            IContentType dataContentType, IContentTypeRegistryService contentTypeRegistryService)
            where TExtensionFactory : class where TMetadataView : INamedContentTypeMetadata
        {
                ...
                if (ExtensionSelector.ContentTypeMatch(dataContentType1, contentTypes))
                {
                    ...
                    else
                    {
                        if (namedFactories == null)
                            namedFactories =
                                new Dictionary<string, List<Lazy<TExtensionFactory, TMetadataView>>>(StringComparer
                                    .OrdinalIgnoreCase);
        ...
        public TExtension InstantiateExtension<TExtension, TMetadata>(object errorSource,
            Lazy<TExtension, TMetadata> provider)
        {
            try
            {
        ...
        public TExtensionInstance InstantiateExtension<TExtension, TMetadata, TExtensionInstance>(object errorSource,
            Lazy<TExtension, TMetadata> provider, Func<TExtension, TExtensionInstance> getter)
        {
            try
            {
        ...
        public TExtensionInstance InvokeBestMatchingFactory<TExtensionFactory, TExtensionInstance, TMetadataView>(
            IList<Lazy<TExtensionFactory, TMetadataView>> providerHandles, IContentType dataContentType,
            Func<TExtensionFactory, TExtensionInstance> getter,
            IContentTypeRegistryService contentTypeRegistryService, object errorSource) where TExtensionFactory : class
            where TMetadataView : IContentTypeMetadata
        {
            var factory = InvokeBestMatchingFactory(providerHandles, dataContentType, contentTypeRegistryService,
                errorSource);
            if (factory == null)
                return default;
        ...
        public TExtension InvokeBestMatchingFactory<TExtension, TMetadataView>(
            IList<Lazy<TExtension, TMetadataView>> providerHandles, IContentType dataContentType,
            IContentTypeRegistryService contentTypeRegistryService, object errorSource)
            where TMetadataView : IContentTypeMetadata
        {
        ...
        public List<TExtensionInstance> InvokeEligibleFactories<TExtensionInstance, TExtensionFactory, TMetadataView>(
            ...
            Func<TExtensionFactory, TExtensionInstance> getter, IContentType dataContentType,
            IContentTypeRegistryService contentTypeRegistryService, object errorSource) where TExtensionInstance : class
            ...
            where TMetadataView : INamedContentTypeMetadata
        {
            ...
            foreach (var eligibleFactory in FindEligibleFactories(lazyFactories, dataContentType,
        ...
        public void RaiseEvent<TArgs>(object sender, EventHandler<TArgs> eventHandlers, TArgs args)
        ...
        private static int ContentTypeScore(string contentTypeName, IReadOnlyList<IContentType> sortedContentTypes)
        {
            for (var index = 0; index < sortedContentTypes.Count; ++index)
                if (string.Compare(sortedContentTypes[index].TypeName, contentTypeName,
                        StringComparison.OrdinalIgnoreCase) == 0)
                    return index;
            return sortedContentTypes.Count;
        }

        private static void SortCandidates<TExtension, TMetadataView>(List<Lazy<TExtension, TMetadataView>> candidates,
            IContentType dataContentType, IContentTypeRegistryService contentTypeRegistryService)
            where TMetadataView : IContentTypeMetadata
        {
