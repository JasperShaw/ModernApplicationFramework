...
namespace ModernApplicationFramework.Modules.Editor.Utilities
{
    internal static class MappingHelper
    {
        ...
        internal static ITextSnapshot FindCorrespondingSnapshot(ITextSnapshot sourceSnapshot,
        ...
        internal static NormalizedSnapshotSpanCollection MapDownToBufferNoTrack(SnapshotSpan sourceSpan,
            ITextBuffer targetBuffer, bool mapByContentType = false)
        {
        ...
        internal static void MapDownToBufferNoTrack(SnapshotSpan sourceSpan, ITextBuffer targetBuffer,
            IList<SnapshotSpan> mappedSpans, bool mapByContentType = false)
        {
            SnapshotSpan snapshotSpan;
            ReadOnlyCollection<SnapshotSpan> sourceSnapshots;
            for (snapshotSpan = sourceSpan;
                snapshotSpan.Snapshot.TextBuffer != targetBuffer;
                snapshotSpan = sourceSnapshots[0])
            {
                if (!(snapshotSpan.Snapshot is IProjectionSnapshot snapshot) ||
                    mapByContentType && !snapshot.ContentType.IsOfType("projection"))
                    return;
                ...
                if (sourceSnapshots.Count != 1)
                {
                    if (sourceSnapshots.Count == 0)
                        return;
                    SplitMapDownToBufferNoTrack(new FrugalList<SnapshotSpan>(sourceSnapshots), targetBuffer,
                        mappedSpans, mapByContentType);
                    return;
                }
            }
            ...
            PositionAffinity affinity)
        {
            ...
            for (;
                position.Snapshot.TextBuffer != targetBuffer;
                position = snapshot.MapToSourceSnapshot(position, affinity))
            {
        ...
        internal static void MapDownToFirstMatchNoTrack(SnapshotSpan sourceSpan, Predicate<ITextBuffer> match,
            IList<SnapshotSpan> mappedSpans, bool mapByContentType = false)
        {
            SnapshotSpan snapshotSpan;
            ReadOnlyCollection<SnapshotSpan> sourceSnapshots;
            for (snapshotSpan = sourceSpan; !match(snapshotSpan.Snapshot.TextBuffer); snapshotSpan = sourceSnapshots[0])
            {
                ...
                if (sourceSnapshots.Count != 1)
                {
                    if (sourceSnapshots.Count == 0)
                        return;
                    SplitMapDownToFirstMatchNoTrack(new FrugalList<SnapshotSpan>(sourceSnapshots), match, mappedSpans,
                        mapByContentType);
                    return;
                }
            }
        ...
        private static void SplitMapDownToBufferNoTrack(FrugalList<SnapshotSpan> unmappedSpans,
            ITextBuffer targetBuffer, IList<SnapshotSpan> mappedSpans, bool mapByContentType)
        {
            while (unmappedSpans.Count > 0)
            {
                ...
                {
                    if (unmappedSpan.Snapshot is IProjectionSnapshot snapshot &&
                        (!mapByContentType || unmappedSpan.Snapshot.TextBuffer.ContentType.IsOfType("projection")))
                        unmappedSpans.AddRange(snapshot.MapToSourceSnapshots(unmappedSpan));
                }
            }
        }

        private static void SplitMapDownToFirstMatchNoTrack(FrugalList<SnapshotSpan> unmappedSpans,
            Predicate<ITextBuffer> match, IList<SnapshotSpan> mappedSpans, bool mapByContentType)
        {
