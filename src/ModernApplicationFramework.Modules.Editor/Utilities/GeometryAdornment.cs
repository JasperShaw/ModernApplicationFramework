...
namespace ModernApplicationFramework.Modules.Editor.Utilities
{
    public class GeometryAdornment : UIElement
    {
        ...
        public GeometryAdornment(Pen borderPen, Brush fillBrush, double left, double top, double width, double height,
            bool isHitTestVisible = false, bool snapsToDevicePixels = false)
            : this(borderPen, fillBrush, new Rect(left, top, width, height), isHitTestVisible, snapsToDevicePixels)
        {
        }

        public GeometryAdornment(Pen borderPen, Brush fillBrush, Rect rect, bool isHitTestVisible = false,
            bool snapsToDevicePixels = false)
            : this(borderPen, fillBrush, new RectangleGeometry(rect), isHitTestVisible, snapsToDevicePixels)
        {
        }

        public GeometryAdornment(Pen borderPen, Brush fillBrush, Geometry geometry, bool isHitTestVisible = false,
            bool snapsToDevicePixels = false)
        {
            BorderPen = borderPen;
            FillBrush = fillBrush;
