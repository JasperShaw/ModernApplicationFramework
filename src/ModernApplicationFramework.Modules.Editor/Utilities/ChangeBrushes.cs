...
namespace ModernApplicationFramework.Modules.Editor.Utilities
{
    ...
    {
        public static NormalizedSnapshotSpanCollection[] GetUnifiedChanges(ITextSnapshot snapshot,
            IEnumerable<IMappingTagSpan<ChangeTag>> tags)
        {
            ...
            {
                var index = (int) (tag.Tag.ChangeTypes &
                                   (ChangeTypes.ChangedSinceOpened | ChangeTypes.ChangedSinceSaved));
                if (index != 0)
            ...
            for (var index = 1; index <= 3; ++index)
                snapshotSpanCollectionArray[index] =
                    new NormalizedSnapshotSpanCollection(snapshot, spanListArray[index]);
