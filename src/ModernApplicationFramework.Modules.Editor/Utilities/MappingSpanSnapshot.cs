...
namespace ModernApplicationFramework.Modules.Editor.Utilities
{
    internal class MappingSpanSnapshot : IMappingSpan
    {
        ...
        public IMappingPoint End => MappingPointSnapshot.Create(_root, _anchor.End,
        ...
        public IMappingPoint Start => MappingPointSnapshot.Create(_root, _anchor.Start,
            _trackingMode == SpanTrackingMode.EdgeInclusive || _trackingMode == SpanTrackingMode.EdgeNegative
                ? PointTrackingMode.Negative
                : PointTrackingMode.Positive, BufferGraph);

        private MappingSpanSnapshot(ITextSnapshot root, SnapshotSpan anchor, SpanTrackingMode trackingMode,
            IBufferGraph graph)
        {
        ...
        public static IMappingSpan Create(ITextSnapshot root, SnapshotSpan anchor, SpanTrackingMode trackingMode,
            IBufferGraph graph)
        {
            return new MappingSpanSnapshot(root, anchor, trackingMode, graph);
        }

        public static void MapUpToSnapshotNoTrack(ITextSnapshot targetSnapshot, SnapshotSpan anchor,
            IList<SnapshotSpan> mappedSpans)
        {
        ...
        public NormalizedSnapshotSpanCollection GetSpans(ITextBuffer targetBuffer)
        {
            ...
            if (targetBuffer.Properties.ContainsProperty("IdentityMapping") &&
                (ITextBuffer) targetBuffer.Properties["IdentityMapping"] == _anchor.Snapshot.TextBuffer)
                return new NormalizedSnapshotSpanCollection(
                    new SnapshotSpan(MappingHelper.FindCorrespondingSnapshot(_root, targetBuffer), _anchor.Span));
