    ...
    internal class NaturalLanguageNavigator : ITextStructureNavigator
    {
        ...
        private static SnapshotPoint FindEndOfSentence(SnapshotPoint currentPosition)
        {
            ...
            while (lineNumber != currentPosition.Snapshot.LineCount)
            {
                ...
                for (var index = (int) lineFromLineNumber.Start <= currentPosition.Position
                        ? currentPosition.Position
                        : lineFromLineNumber.Start;
                    index < lineFromLineNumber.EndIncludingLineBreak;
                    ++index)
        ...
        private static SnapshotPoint FindEndOfWord(SnapshotPoint currentPosition, ITextSnapshotLine snapshotLine,
            PseudoString bufferLineText, CharacterType baseType)
        {
        ...
        private static SnapshotPoint FindStartOfSentence(SnapshotPoint currentPosition)
        {
            ...
            while (lineNumber >= 0)
            {
                ...
                if (lineFromLineNumber.Length == 0)
                    return lineFromLineNumber.EndIncludingLineBreak;
                var pseudoString = new PseudoString(lineFromLineNumber);
                for (var index = lineFromLineNumber.EndIncludingLineBreak - 1 < currentPosition.Position
                        ? lineFromLineNumber.EndIncludingLineBreak - 1
                        : currentPosition.Position - 1;
                    index >= lineFromLineNumber.Start;
                    --index)
                    if (".!?".IndexOf(pseudoString[index - lineFromLineNumber.Start]) != -1)
                        return new SnapshotPoint(currentPosition.Snapshot, index + 1);
                if (lineFromLineNumber.Start == 0)
        ...
        private static SnapshotPoint FindStartOfWord(SnapshotPoint currentPosition, ITextSnapshotLine snapshotLine,
            PseudoString bufferLineText, CharacterType baseType)
        {
        ...
        private static CharacterType GetCharacterType(char c)
        {
            ...
            return char.IsSurrogate(c) || UnicodeCategory.NonSpacingMark == char.GetUnicodeCategory(c)
