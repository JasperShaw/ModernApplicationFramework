...
namespace ModernApplicationFramework.Modules.Editor.Outlining
{
    internal sealed class OutliningManager : IAccurateOutliningManager
    {
        ...
        internal OutliningManager(ITextBuffer editBuffer, ITagAggregator<IOutliningRegionTag> tagAggregator,
        ...
        public IEnumerable<ICollapsible> ExpandAllInternal(bool removalPending, SnapshotSpan span,
        ...
        internal IList<Collapsed> GetCollapsedRegionsInternal(NormalizedSnapshotSpanCollection spans,
            bool exposedRegionsOnly)
        {
            EnsureValid(spans);
            if (!_isEnabled)
        ...
        internal IEnumerable<ICollapsed> InternalCollapseAll(SnapshotSpan span, Predicate<ICollapsible> match,
            CancellationToken? cancel)
        {
            if (match == null)
                throw new ArgumentNullException(nameof(match));
        ...
        internal IEnumerable<ICollapsible> InternalGetAllRegions(NormalizedSnapshotSpanCollection spans,
            bool exposedRegionsOnly, CancellationToken? cancel = null)
        {
            EnsureValid(spans);
            ...
            {
                var right = new NormalizedSnapshotSpanCollection(
                    collapsedRegionsInternal.Select(c => c.Extent.GetSpan(snapshot)));
                newCollapsibles = CollapsiblesFromTags(InternalGetTags(NormalizedSnapshotSpanCollection.Union(
                    NormalizedSnapshotSpanCollection.Difference(spans, right), new NormalizedSnapshotSpanCollection(
                        new SnapshotSpan[2]
                        {
        ...
        private void AvoidPartialLinebreaks(TextContentChangedEventArgs args)
        {
            ...
            if (!flag)
                return;
            ExpandAll(
                new SnapshotSpan(args.After,
                    Span.FromBounds(args.Changes[0].NewPosition, args.Changes[args.Changes.Count - 1].NewEnd)),
                collapsed => true);
        }
        ...
        private SortedList<Collapsible, object> CollapsiblesFromTags(
            IEnumerable<IMappingTagSpan<IOutliningRegionTag>> tagSpans)
        {
            var currentSnapshot = _editBuffer.CurrentSnapshot;
            ...
            foreach (var tagSpan in tagSpans)
            {
                ...
                if (spans.Count == 1)
                {
                    ...
                    if (span.Length > 0)
                    {
                        ...
                        if (length1 == length2)
                        {
                            var key = new Collapsible(
                                currentSnapshot.CreateTrackingSpan(spans[0], SpanTrackingMode.EdgeExclusive),
                                tagSpan.Tag);
        ...
        private void EnsureValid(NormalizedSnapshotSpanCollection spans)
        {
            ...
            if (spans[0].Snapshot.TextBuffer != _editBuffer)
                throw new ArgumentException(
                    "The given span collection is on an invalid buffer.Spans must be generated against the view model's edit buffer",
                    nameof(spans));
        }
        ...
        private ICollapsible ExpandInternal(ICollapsed collapsed)
        {
            ...
            if (!collapsed1.IsValid)
                throw new InvalidOperationException(
                    "The collapsed region is invalid, meaning it has already been expanded.");
            if (!_collapsedRegionTree.RemoveItem(collapsed1, collapsed1.Extent))
                throw new ApplicationException(
                    "Unable to remove the collapsed region from outlining manager, which means there is an internal consistency issue.");
            collapsed1.Invalidate();
            return new Collapsible(collapsed.Extent, collapsed.Tag);
        }

        private IEnumerable<IMappingTagSpan<IOutliningRegionTag>> InternalGetTags(
            NormalizedSnapshotSpanCollection spans, CancellationToken? cancel)
        {
        ...
        private IEnumerable<ICollapsible> MergeRegions(IEnumerable<ICollapsed> currentCollapsed,
            IEnumerable<ICollapsible> newCollapsibles, out IEnumerable<ICollapsed> removedRegions)
        {
            ...
            var index1 = 0;
            var index2 = 0;
            var collapsibleSorter = new CollapsibleSorter(_editBuffer);
            while (index1 < collapsedList2.Count || index2 < collapsibleList1.Count)
                if (index1 < collapsedList2.Count && index2 < collapsibleList1.Count)
                {
                    ...
                    var num = collapsibleSorter.Compare(collapsed, y);
                    if (num == 0)
                    {
                        collapsed.Tag = y.Tag;
        ...
        private void OutliningRegionTagsChanged(object sender, BatchedTagsChangedEventArgs e)
        {
            if (!_isEnabled)
                return;
            UpdateAfterChange(new NormalizedSnapshotSpanCollection(e.Spans.SelectMany(s =>
                s.GetSpans(_editBuffer) as IEnumerable<SnapshotSpan>)));
        }
        ...
        private void UpdateAfterChange(NormalizedSnapshotSpanCollection changedSpans)
        {
            ...
            if (collapsedRegionsInternal.Count > 0)
            {
                ...
                var keys = CollapsiblesFromTags(_tagAggregator.GetTags(
                    NormalizedSnapshotSpanCollection.Intersection(changedSpans,
                        new NormalizedSnapshotSpanCollection(
                            collapsedRegionsInternal.Select(c => c.Extent.GetSpan(snapshot)))))).Keys;
            ...
            regionsChanged?.Invoke(this,
                new RegionsChangedEventArgs(new SnapshotSpan(changedSpans[0].Start,
                    changedSpans[changedSpans.Count - 1].End)));
        }
    }
}