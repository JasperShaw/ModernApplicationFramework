...
namespace ModernApplicationFramework.Modules.Editor.Formatting
{
    ...
    internal sealed class FormattedTextSourceFactoryService : IFormattedTextSourceFactoryService
    {
        ...
        [Import] private IClassificationTypeRegistryService ClassificationTypeRegistry { get; set; }

        [Import] private IContentTypeRegistryService ContentTypeRegistry { get; set; }

        [Import] private GuardedOperations GuardedOperations { get; set; }

        [ImportMany]
        private List<Lazy<ITextParagraphPropertiesFactoryService, IContentTypeMetadata>> ParagraphPropertiesFactories
        {
            get;
            set;
        }

        public IFormattedLineSource Create(ITextSnapshot sourceTextSnapshot, ITextSnapshot visualBufferSnapshot,
            int tabSize, double baseIndentation, double wordWrapWidth, double maxAutoIndent, bool useDisplayMode,
            IClassifier aggregateClassifier, ITextAndAdornmentSequencer sequencer,
            IClassificationFormatMap classificationFormatMap)
        {
            return Create(sourceTextSnapshot, visualBufferSnapshot, tabSize, baseIndentation, wordWrapWidth,
                maxAutoIndent, useDisplayMode, aggregateClassifier, sequencer, classificationFormatMap, false);
        }
            ...
            IClassificationFormatMap classificationFormatMap, bool isViewWrapEnabled)
        {
            ...
            var viewWrapProperties = isViewWrapEnabled
                ? new ViewWrapProperties(textInfo.SpaceWidth * 1.5,
                    classificationFormatMap
                        .GetTextProperties(ClassificationTypeRegistry.GetClassificationType("word wrap glyph"))
                        .ForegroundBrush)
                : null;
            return new FormattedLineSource(sourceTextSnapshot, visualBufferSnapshot, tabSize, textInfo, baseIndentation,
                wordWrapWidth, maxAutoIndent, aggregateClassifier, sequencer, classificationFormatMap,
                viewWrapProperties, textInfoCache, ParagraphPropertiesFactories, GuardedOperations,
                ContentTypeRegistry);
        }
        ...
        {
            return Create(sourceTextSnapshot, visualBufferSnapshot, tabSize, baseIndentation, wordWrapWidth,
                maxAutoIndent, useDisplayMode, new EmptyClassifier(), sequencer, classificationFormatMap);
        }

        internal void Initialize(IClassificationTypeRegistryService classificationTypeRegistry,
            List<Lazy<ITextParagraphPropertiesFactoryService, IContentTypeMetadata>> paragraphPropertiesFactories)
        {
