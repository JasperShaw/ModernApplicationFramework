...
namespace ModernApplicationFramework.Modules.Editor.Formatting
{
    internal sealed class FormattedLine : IFormattedLine
    {
        ...
        public SnapshotSpan Extent
        {
            ...
            {
                ThrowIfInvalid();
                return new SnapshotSpan(_extentIncludingLineBreak.Snapshot, _extentIncludingLineBreak.Start,
                    _extentIncludingLineBreak.Length - _lineBreakLength);
            }
        }

        public IMappingSpan ExtentAsMappingSpan => MappingSpanSnapshot.Create(_visualSnapshot,
            new SnapshotSpan(
                MappingPointSnapshot.MapUpToSnapshotNoTrack(_visualSnapshot, _extentIncludingLineBreak.Start,
                    PositionAffinity.Predecessor).Value,
                MappingPointSnapshot.MapUpToSnapshotNoTrack(_visualSnapshot,
                    _extentIncludingLineBreak.End - _lineBreakLength, PositionAffinity.Successor).Value),
            SpanTrackingMode.EdgeInclusive, _bufferGraph);
        ...
        public IMappingSpan ExtentIncludingLineBreakAsMappingSpan => MappingSpanSnapshot.Create(_visualSnapshot,
            new SnapshotSpan(
                MappingPointSnapshot.MapUpToSnapshotNoTrack(_visualSnapshot, _extentIncludingLineBreak.Start,
                    PositionAffinity.Predecessor).Value,
                MappingPointSnapshot.MapUpToSnapshotNoTrack(_visualSnapshot, _extentIncludingLineBreak.End,
                    PositionAffinity.Successor).Value), SpanTrackingMode.EdgeInclusive, _bufferGraph);
        ...
        public FormattedLine(double indentation, IList<TextLineData> textLines, IBufferGraph bufferGraph,
            ITextSnapshot visualSnapshot, NormalizedSpanTextSource textSource,
            BufferPositionToTokenIndexMap bufferToTokenMap, int startTokenIndex, int endTokenIndex, int lineBreakLength,
            bool isFirstTextViewLineForSnapshotLine, bool isLastTextViewLineForSnapshotLine,
            ViewWrapProperties viewWrapProperties, TextInfoCache textInfoCache, double defaultSpaceWidth)
        {
            ...
            do
            {
                ...
                if (normalizedSpan.ClassifiedRun != null)
                {
                    ...
                    _mayContainMultiCharacterGlyphs = _mayContainMultiCharacterGlyphs ||
                                                      normalizedSpan.ClassifiedRun.MayContainMultiCharacterGlyphs;
                ...
                else if (normalizedSpan.Element != null)
                {
                    ...
                    if (normalizedSpan.Element.ProviderTag != null)
                    {
                        ...
                        if (!_providerToAdornmentMap.TryGetValue(normalizedSpan.Element.ProviderTag,
                            out var frugalList))
                        {
        ...
        public ReadOnlyCollection<object> GetAdornmentTags(object providerTag)
        {
            ...
            if (_providerToAdornmentMap == null ||
                !_providerToAdornmentMap.TryGetValue(providerTag, out var frugalList))
        ...
        public SnapshotPoint? GetBufferPositionFromXCoordinate(double xCoordinate, bool textOnly)
        {
            ...
            foreach (var textLine in _textLines)
                ...
                {
                    var firstCharacterIndex = textLine.TextLine.GetCharacterHitFromDistance(xCoordinate - textLine.Left)
                        .FirstCharacterIndex;
                    if (textOnly && !_bufferToTokenMap.IsTokenIndexABufferPosition(firstCharacterIndex))
                        return new SnapshotPoint?();
                    return new SnapshotPoint(_bufferToTokenMap.SourceTextSnapshot,
                        Math.Max(Start.Position,
                            Math.Min(End.Position,
                                _bufferToTokenMap.GetAssociatedBufferPositionFromTokenIndex(firstCharacterIndex))));
                }
        ...
        public TextBounds GetCharacterBounds(VirtualSnapshotPoint virtualBufferPosition)
        {
            ...
            if (IsLastTextViewLineForSnapshotLine)
                return new TextBounds(_textRight + virtualBufferPosition.VirtualSpaces * _virtualSpaceWidth, _top,
                    _virtualSpaceWidth, _height, TextTop, TextHeight);
        ...
        public TextBounds GetExtendedCharacterBounds(SnapshotPoint bufferPosition)
        {
            ...
            {
                var leading = !IsLastTextViewLineForSnapshotLine
                    ? _textRight
                    : GetTokenBounds(
                        _bufferToTokenMap.GetLeftmostTokenIndexForBufferPosition(
                            _extentIncludingLineBreak.End.Position - _lineBreakLength)).Left;
            ...
            var num1 = !_bufferToTokenMap.GetElisionSpan(textElementSpan.Start).HasValue
                ? fromBufferPosition + textElementSpan.Length - 1
                : fromBufferPosition;
            ...
            var forBufferPosition2 =
                _bufferToTokenMap.GetRightmostTokenIndexForBufferPosition(textElementSpan.End.Position - 1);
            ...
            return new TextBounds(leading1, tokenBounds.Top, num2 - leading1, tokenBounds.Height, tokenBounds.TextTop,
                tokenBounds.TextHeight);
        }
        ...
        public VirtualSnapshotPoint GetInsertionBufferPositionFromXCoordinate(double x)
        {
            ...
            if (nullable.HasValue)
            {
                ...
                if (position < num)
                {
                    ...
                    if (extendedCharacterBounds.IsRightToLeft ==
                        x < extendedCharacterBounds.Left + extendedCharacterBounds.Width * 0.5)
        ...
        public SnapshotSpan GetTextElementSpan(SnapshotPoint bufferPosition)
        {
            ...
            return new SnapshotSpan(_bufferToTokenMap.SourceTextSnapshot,
                _bufferToTokenMap.GetNextBufferPositionFromTokenIndex(firstCharacterIndex2),
                firstCharacterIndex1 - firstCharacterIndex2);
        }
        ...
        public void SetVisibleArea(Rect visibleArea)
        {
            ...
            {
                if (_visual != null && (_visibleArea.Left < _visual.RenderedLeftEdge ||
                                        _visibleArea.Right > _visual.RenderedRightEdge))
                    _visual.RenderText();
        ...
        private TextBounds GetTokenBounds(int tokenIndex)
        {
            ...
            var fromCharacterHit1 =
                containingTokenIndex.TextLine.GetDistanceFromCharacterHit(new CharacterHit(tokenIndex, 0));
            var fromCharacterHit2 =
                containingTokenIndex.TextLine.GetDistanceFromCharacterHit(new CharacterHit(tokenIndex, 1));
            return new TextBounds(containingTokenIndex.Left + fromCharacterHit1, Top,
                fromCharacterHit2 - fromCharacterHit1, Height, TextTop, TextHeight);
        }

        private List<Tuple<double, double>> GetUnnormalizedBounds(SnapshotPoint startElementPosition,
            SnapshotPoint endElementPosition)
        {
            ...
            for (var textLineIndex = containingTokenIndex1.TextLineIndex;
                textLineIndex <= containingTokenIndex2.TextLineIndex;
                ++textLineIndex)
            {
                ...
                if (firstTextSourceCharacterIndex < num2)
                {
                    if (firstTextSourceCharacterIndex == textLine.TokenSpan.Start && num2 == textLine.TokenSpan.End)
                        ...
                        foreach (var textBound in textLine.TextLine.GetTextBounds(firstTextSourceCharacterIndex,
                            num2 - firstTextSourceCharacterIndex))
                        {
        ...
        {
            _extentIncludingLineBreak = new SnapshotSpan(_bufferToTokenMap.SourceTextSnapshot,
                Span.FromBounds(_bufferToTokenMap.GetNextBufferPositionFromTokenIndex(_tokenIndexSpan.Start),
                    _bufferToTokenMap.GetNextBufferPositionFromTokenIndex(_tokenIndexSpan.End)));
        }
        ...
        internal class RenderedLineVisual : DrawingVisual
        {
            ...
            {
                _transform = new MatrixTransform(1.0, 0.0, 0.0, _textViewLine.LineTransform.VerticalScale, 0.0,
                    _textViewLine.TextTop);
                _transform.Freeze();
            ...
            private static void DrawViewWrapIndicator(DrawingContext drawingContext, FormattedLine line, double lineTop)
            {
                ...
                var arcSegment =
                    new ArcSegment(new Point(num1 + width / 2.0, num2 + textHeight / 2.0 + textHeight / 8.0),
                        new Size(width / 2.0, textHeight / 8.0), 180.0, false, SweepDirection.Clockwise, true);
                arcSegment.Freeze();
                var pathFigure1 = new PathFigure(
                    new Point(num1 + width / 8.0, num2 + textHeight / 2.0 - textHeight / 8.0), new PathSegment[]
                    {
                        arcSegment
                    }, false);
                pathFigure1.Freeze();
                ...
                var pathFigure2 = new PathFigure(
                    new Point(num1 + width / 2.0 - textHeight / 8.0, num2 + textHeight / 2.0 + textHeight / 8.0),
                    new PathSegment[]
                    {
                        polyLineSegment
                    }, true);
