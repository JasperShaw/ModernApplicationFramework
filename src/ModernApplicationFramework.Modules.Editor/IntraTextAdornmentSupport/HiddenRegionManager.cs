...
namespace ModernApplicationFramework.Modules.Editor.IntraTextAdornmentSupport
{
    internal sealed class HiddenRegionManager : IDisposable
    {
        ...
        internal HiddenRegionManager(IElisionBuffer elisionBuffer, ITagAggregator<IElisionTag> hiddenRegionTagAggregator)
        {
        ...
        private void UpdateAfterChange(SnapshotSpan changedSpan)
        {
            ...
            if (changedSpan.Length == 0)
                return;
            var list = _hiddenRegionTagAggregator.GetTags(changedSpan).SelectMany(mappingTag => mappingTag.Span.GetSpans(snap) as IEnumerable<SnapshotSpan>).ToList();
            var right = NormalizedSnapshotSpanCollection.Intersection(new NormalizedSnapshotSpanCollection(list), new NormalizedSnapshotSpanCollection(changedSpan));
            var snapshotSpanCollection1 = NormalizedSnapshotSpanCollection.Difference(new NormalizedSnapshotSpanCollection(changedSpan), right);
            if (list.Count > 1)
            {
                ...
                snapshotSpanCollection1 = NormalizedSnapshotSpanCollection.Union(new NormalizedSnapshotSpanCollection(snapshotSpanList), snapshotSpanCollection1);
            }
            var snapshotSpanCollection2 = NormalizedSnapshotSpanCollection.Union(snapshotSpanCollection1, new NormalizedSnapshotSpanCollection(new[]
            {
