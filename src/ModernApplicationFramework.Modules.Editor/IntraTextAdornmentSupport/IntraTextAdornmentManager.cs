...
namespace ModernApplicationFramework.Modules.Editor.IntraTextAdornmentSupport
{
    ...
    {
        internal static readonly DependencyProperty ScalingElementContainerProperty =
            DependencyProperty.RegisterAttached("ScalingElementContainer", typeof(ScalingElementContainer),
                typeof(UIElement));

        private readonly ITagAggregator<IntraTextAdornmentTag> _adornmentTagAggregator;

        private readonly List<IMappingTagSpan<IntraTextAdornmentTag>> _displayedAdornments =
            new List<IMappingTagSpan<IntraTextAdornmentTag>>();
        ...
        private IEnumerable<ITagSpan<IElisionTag>> GetHiddenRegionTags(NormalizedSnapshotSpanCollection spans)
        {
            if (spans.Count != 0)
                ...
                foreach (var tag in _adornmentTagAggregator.GetTags(span))
                {
                    ...
                    {
                        var singleSpan = new SnapshotSpan(adornmentSpans[0].Start,
                            adornmentSpans[adornmentSpans.Count - 1].End);
                        yield return new TagSpan<IElisionTag>(singleSpan, new ElisionTag());
                    }
                }
        }

        private IEnumerable<ITagSpan<SpaceNegotiatingAdornmentTag>> GetSpaceNegotiationTags(
            NormalizedSnapshotSpanCollection spans)
        {
            if (spans.Count != 0)
                ...
                foreach (var tag1 in _adornmentTagAggregator.GetTags(span))
                {
                    ...
                    {
                        var singleSpan = new SnapshotSpan(adornmentSourceSpans[0].Start,
                            adornmentSourceSpans[adornmentSourceSpans.Count - 1].End);
                        ...
                        var tag = new SpaceNegotiatingAdornmentTag(width, topSpace1, baseline1, textHeight1,
                            bottomSpace1, (PositionAffinity) num, uiElement, this);
                ...
                if (line != null && line.IsValid)
                {
                    ...
                    foreach (var tag in adornmentTags)
                        if (tag is UIElement uiElement)
                            if (uiElement.GetValue(ScalingElementContainerProperty) is ScalingElementContainer
                                    elementContainer1 && !elementContainer1.IsAddedToLayer)
                            {
                                var adornmentBounds1 = line.GetAdornmentBounds(tag);
                                if (adornmentBounds1.HasValue)
                                {
                                    Canvas.SetTop(elementContainer1,
                                        adornmentBounds1.Value.TextTop + line.Baseline -
                                        line.LineTransform.VerticalScale * elementContainer1.Baseline);
                                    ...
                                    var adornmentSourceSpan =
                                        GetSingleSpan(elementContainer1.Span.Span.GetSpans(_view.TextSnapshot));
                                    var selectionOnTextViewLine = _view.Selection.GetSelectionOnTextViewLine(line);
                                    var isSelected = selectionOnTextViewLine.HasValue &&
                                                     selectionOnTextViewLine.Value.SnapshotSpan.Contains(
                                                         adornmentSourceSpan);
                                    IntraTextAdornment.SetIsSelected(elementContainer1, isSelected);
                                    var capturedLine = line;

                                    void SizeChangedEventHandler(object senderAdornment,
                                        SizeChangedEventArgs sizeChangedArgs)
                                    {
                                        ...
                                        _spaceNegotiationTagger.RaiseTagsChanged(
                                            new SnapshotSpanEventArgs(new SnapshotSpan(start, end)));
                                    }

                                    elementContainer1.AddToLayer();
                                    AdornmentLayer.AddAdornment(AdornmentPositioningBehavior.TextRelative,
                                        adornmentSourceSpan, null, elementContainer1, (adornmentIdentity, element) =>
                                        {
        ...
        private void HandleSelectionChanged(object sender, EventArgs args)
        {
            foreach (var displayedAdornment in _displayedAdornments)
            {
                ...
                var containingBufferPosition =
                    _view.TextViewLines.GetTextViewLineContainingBufferPosition(singleSpan.Start);
                if (containingBufferPosition != null)
                {
                    ...
                    var isSelected = selectionOnTextViewLine.HasValue &&
                                     selectionOnTextViewLine.Value.SnapshotSpan.Contains(singleSpan);
