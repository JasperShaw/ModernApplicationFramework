...
namespace ModernApplicationFramework.Modules.Editor.UrlTagger
{
    internal static class UrlUtility
    {
            ...
            while ((end = text.IndexOf(':', end + 1)) != -1)
                if (end > lastColonIndex + 2)
                {
                    ...
                    if (hitOffset.HasValue && startIndex > hitOffset.Value)
                        break;
                    ++startIndex;
                    if (IsProtocol(text.Substring(startIndex,
                        Math.Min(text.Length - startIndex, end - startIndex + 3))))
                    {
                        ...
                        if (!IsInvalidUrlCharacter(text[addressStart]))
                        {
                            ...
                            if (index1 != -1)
                            {
                                ...
                                if (lastCharInUrl != -1)
                                {
                                    ...
                                    if (!hitOffset.HasValue || startIndex - 1 <= hitOffset.Value &&
                                        hitOffset.Value <= lastCharInUrl)
                                    {
                            ...
                            label_32:
                            if (lastCharInUrl != end &&
                                (!hitOffset.HasValue ||
                                 startIndex <= hitOffset.Value && hitOffset.Value <= lastCharInUrl))
                            {
                                yield return new UrlSpan
                                {
        ...
        {
            if (possibleProtocol.Length < 2 || possibleProtocol.Length > 7 ||
                !StandardProtocols.TryGetValue(possibleProtocol, out var flag))
