...
namespace ModernApplicationFramework.Modules.Editor.Classification
{
    internal sealed class ClassificationFormatMap : IClassificationFormatMap
    {
        ...
        private readonly Dictionary<string, TextFormattingRunProperties> _textFormattingPropertiesWithDefaults =
        ...
        public ClassificationFormatMap(ICollection<Lazy<EditorFormatDefinition, IClassificationFormatMetadata>> formats,
            IClassificationTypeRegistryService classificationTypeRegistry, IEditorFormatMap formatMap,
            IEditorOptionsFactoryService optionsService)
        {
            Formats = formats;
        ...
        public TextFormattingRunProperties GetExplicitTextProperties(IClassificationType classificationType)
        {
            ...
            return CreateTfrpFromRd(_formatMap.GetProperties(GetEditorFormatMapKey(classificationType)),
                isInContrastMode);
        }
        ...
        public TextFormattingRunProperties GetTextProperties(IClassificationType classificationType)
        {
            if (classificationType == null)
                throw new ArgumentNullException(nameof(classificationType));
            if (_textFormattingPropertiesWithDefaults.TryGetValue(classificationType.Classification,
                out var formattingRunProperties))
                return formattingRunProperties;
            formattingRunProperties = CreateTfrpFromRd(GetResourceDictionaryFromHierarchy(classificationType),
                _optionsService.GlobalOptions.IsInContrastMode());
        ...
        internal string GetFormatNameFromClassification(string classification)
        {
            ...
            foreach (var format in Formats)
                if (format.Metadata.ClassificationTypeNames.Any(classificationTypeName =>
                    string.Compare(classificationTypeName, classification, StringComparison.OrdinalIgnoreCase) == 0))
                {
        ...
        private static TextFormattingRunProperties CreateTfrpFromRd(ResourceDictionary dictionary,
            bool isInContrastMode)
        {
            ...
            if (background != null)
            {
                ...
                if (!isInContrastMode)
                    background.Opacity = !dictionary.Contains("BackgroundOpacity")
                        ? 0.8
                        : (double) dictionary["BackgroundOpacity"];
            }
            ...
            if (typeface != null)
            {
                var weight = nullable1.HasValue
                    ? (nullable1.Value ? FontWeights.Bold : FontWeights.Normal)
                    : typeface.Weight;
                var style = nullable2.HasValue
                    ? (nullable2.Value ? FontStyles.Italic : FontStyles.Normal)
                    : typeface.Style;
                if (weight != typeface.Weight || style != typeface.Style)
                    typeface = new Typeface(typeface.FontFamily, style, weight, typeface.Stretch,
                        new FontFamily("Global Monospace, Global User Interface"));
            }
            ...
            var formattingRunProperties = TextFormattingRunProperties.CreateTextFormattingRunProperties(foreground,
                background, typeface, size, hintingSize, textDecorations, textEffects, cultureInfo);
            if (foreground == null)
        ...
        private void BuildOrderedClassifications(IClassificationType classificationType,
            List<IClassificationType> orderedClassifications)
        {
            ...
            if (classificationTypeList.Count > 1)
                classificationTypeList.Sort(
                    (p1, p2) => _priorityOrder.IndexOf(p2).CompareTo(_priorityOrder.IndexOf(p1)));
        ...
        private void CreateFormatPriorityOrder(
            IEnumerable<Lazy<EditorFormatDefinition, IClassificationFormatMetadata>> orders)
        {
        ...
        private void EnsureDefaultTextProperties()
        {
            if (!_defaultTextProperties.Contains("Typeface"))
                _defaultTextProperties["Typeface"] = new Typeface(new FontFamily("Consolas"), FontStyles.Normal,
            ...
            _defaultTextFormattingRunProperties = CreateTfrpFromRd(_defaultTextProperties,
                _optionsService.GlobalOptions.IsInContrastMode());
        }
