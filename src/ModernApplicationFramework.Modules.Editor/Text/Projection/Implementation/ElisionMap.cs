...
using TextChange = ModernApplicationFramework.Modules.Editor.Text.Implementation.TextChange;
    ...
    internal class ElisionMap
    {
        ...
        public ElisionMap(ITextSnapshot sourceSnapshot, NormalizedSpanCollection exposedSpans)
        {
            ...
            else
            {
                ...
                _root = Build(new SnapshotSpan(sourceSnapshot, 0, sourceSnapshot.Length), exposedSpans, lineNumbers, new Span(0, exposedSpans.Count));
            }
        ...
        private ElisionMapNode Build(SnapshotSpan sourceSpan, NormalizedSpanCollection exposedSpans, int[] lineNumbers, Span slice)
        {
            ...
            else if (slice.Start == 0 && exposedSpan.Start != 0)
            {
                ...
                left = new ElisionMapNode(0, span1.Length, 0, TextUtilities.ScanForLineCount(sourceSpan.Snapshot.GetText(span1)), true);
            }
            else
            {
            ...
            return new ElisionMapNode(exposedSpan.Length, sourceSpan.Length - (span1.Length + span2.Length), exposedLineBreakCount, exposedLineBreakCount + num2, left, right, false);
        }
        ...
        public ElisionMap EditSpans(ITextSnapshot sourceSnapshot, NormalizedSpanCollection spansToElide, NormalizedSpanCollection spansToExpand, out FrugalList<TextChange> textChanges)
        {
            textChanges = new FrugalList<TextChange>();
            NormalizedSpanCollection left = new NormalizedSnapshotSpanCollection(GetSourceSpans(sourceSnapshot, 0, SpanCount));
            ...
            foreach (var span in NormalizedSpanCollection.Difference(left, normalizedSpanCollection1))
                textChanges.Add(TextChange.Create(_root.MapFromSourceSnapshotToNearest(span.Start, 0), BufferFactoryService.StringRebuilderFromSnapshotAndSpan(sourceSnapshot, span), StringRebuilder.Empty, sourceSnapshot));
            var normalizedSpanCollection2 = NormalizedSpanCollection.Union(normalizedSpanCollection1, spansToExpand);
            foreach (var span in NormalizedSpanCollection.Difference(normalizedSpanCollection2, normalizedSpanCollection1))
                textChanges.Add(TextChange.Create(_root.MapFromSourceSnapshotToNearest(span.Start, 0), StringRebuilder.Empty, BufferFactoryService.StringRebuilderFromSnapshotAndSpan(sourceSnapshot, span), sourceSnapshot));
        ...
        public IList<SnapshotSpan> GetSourceSpans(ITextSnapshot sourceSnapshot, int startSpanIndex, int count)
        {
            ...
            _root.GetSourceSpans(sourceSnapshot, ref rank, ref sourcePrefixSize, startSpanIndex, startSpanIndex + count, frugalList);
            return frugalList;
        }
        ...
        private void MapNullSpansToSourceSnapshots(IElisionSnapshot elisionSnapshot, Span span, FrugalList<SnapshotSpan> result)
        {
        ...
        public void MapToSourceSnapshotsInFillInMode(ITextSnapshot sourceSnapshot, Span span, FrugalList<SnapshotSpan> result)
        {
            ...
            if (span.Length == 0)
            {
                nullable1 = span.Start == 0 ? new SnapshotPoint(sourceSnapshot, 0) : _root.MapToSourceSnapshot(sourceSnapshot, span.Start, 0, PositionAffinity.Predecessor);
                nullable2 = span.End == Length ? new SnapshotPoint(sourceSnapshot, sourceSnapshot.Length) : _root.MapToSourceSnapshot(sourceSnapshot, span.End, 0, PositionAffinity.Successor);
            }
            else
            {
        ...
        public ElisionMap IncorporateChanges(INormalizedTextChangeCollection sourceChanges, FrugalList<TextChange> projectedChanges, ITextSnapshot beforeSourceSnapshot, ITextSnapshot sourceSnapshot, ITextSnapshot beforeElisionSnapshot)
        {
            ...
            foreach (var sourceChange in sourceChanges)
            {
                ...
                root = root.IncorporateChange(beforeSourceSnapshot, sourceSnapshot, beforeElisionSnapshot, sourceChange.NewLength > 0 ? sourceChange.NewPosition : new int?(), newText, new Span(sourceChange.NewPosition, sourceChange.OldLength), sourceChange.OldPosition, sourceChange.NewPosition, 0, projectedChanges, incomingAccumulatedDelta, ref outgoingAccumulatedDelta, ref accumulatedDelete);
