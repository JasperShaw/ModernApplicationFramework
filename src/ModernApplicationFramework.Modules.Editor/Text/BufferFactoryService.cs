...
namespace ModernApplicationFramework.Modules.Editor.Text
{
    ...
    internal class BufferFactoryService : ITextBufferFactoryService, IProjectionBufferFactoryService,
        IInternalTextBufferFactory, ITextImageFactoryService
    {
        [Export] [Name("any")] public ContentTypeDefinition AnyContentTypeDefinition;

        [Export] [Name("code")] [BaseDefinition("text")]
        public ContentTypeDefinition CodeContentType;

        [Export] [Name("inert")] public ContentTypeDefinition InertContentTypeDefinition;

        [Export] [Name("plaintext")] [BaseDefinition("text")]
        public ContentTypeDefinition PlaintextContentTypeDefinition;

        [Export] [Name("projection")] [BaseDefinition("any")]
        public ContentTypeDefinition ProjectionContentTypeDefinition;

        [Export] [Name("text")] [BaseDefinition("any")]
        ...
        public IContentType InertContentType => _inertContentType ??
                                                (_inertContentType =
                                                    ContentTypeRegistryService.GetContentType("inert"));

        public IContentType PlaintextContentType => _plaintextContentType ??
                                                    (_plaintextContentType =
                                                        ContentTypeRegistryService.GetContentType("plaintext"));

        public IContentType ProjectionContentType => _projectionContentType ??
                                                     (_projectionContentType =
                                                         ContentTypeRegistryService.GetContentType("projection"));

        public IContentType TextContentType =>
            _textContentType ?? (_textContentType = ContentTypeRegistryService.GetContentType("text"));

        [Import] internal IContentTypeRegistryService ContentTypeRegistryService { get; set; }

        [Import] internal IDifferenceService DifferenceService { get; set; }

        [Import] internal GuardedOperations GuardedOperations { get; set; }

        [Import] internal ITextDifferencingSelectorService TextDifferencingSelectorService { get; set; }
            ...
            NormalizedSnapshotSpanCollection exposedSpans, ElisionBufferOptions options, IContentType contentType)
        {
            ...
            if (exposedSpans[0].Snapshot != exposedSpans[0].Snapshot.TextBuffer.CurrentSnapshot)
                throw new ArgumentException(
                    "Elision buffer must be created against the current snapshot of its source buffer");
            IElisionBuffer elisionBuffer = new ElisionBuffer(projectionEditResolver, contentType,
                exposedSpans[0].Snapshot.TextBuffer, exposedSpans, options,
                TextDifferencingSelectorService.DefaultTextDifferencingService, GuardedOperations);
        ...
        public IProjectionBuffer CreateProjectionBuffer(IProjectionEditResolver projectionEditResolver,
            IList<object> trackingSpans, ProjectionBufferOptions options, IContentType contentType)
        {
            if (trackingSpans == null)
                throw new ArgumentNullException(nameof(trackingSpans));
            if (contentType == null)
                throw new ArgumentNullException(nameof(contentType));
            IProjectionBuffer projectionBuffer = new ProjectionBuffer(this, projectionEditResolver, contentType,
                trackingSpans, DifferenceService, TextDifferencingSelectorService.DefaultTextDifferencingService,
                options, GuardedOperations);
            RaiseProjectionBufferCreatedEvent(projectionBuffer);
            return projectionBuffer;
        }

        public IProjectionBuffer CreateProjectionBuffer(IProjectionEditResolver projectionEditResolver,
            IList<object> trackingSpans, ProjectionBufferOptions options)
        {
            if (trackingSpans == null)
                throw new ArgumentNullException(nameof(trackingSpans));
            IProjectionBuffer projectionBuffer = new ProjectionBuffer(this, projectionEditResolver,
                ProjectionContentType, trackingSpans, DifferenceService,
                TextDifferencingSelectorService.DefaultTextDifferencingService, options, GuardedOperations);
            RaiseProjectionBufferCreatedEvent(projectionBuffer);
            return projectionBuffer;
        }
        ...
        internal static StringRebuilder AppendStringRebuildersFromSnapshotAndSpan(StringRebuilder content,
            ITextSnapshot snapshot, Span span)
        {
            var baseSnapshot = snapshot as BaseSnapshot;
            content = baseSnapshot == null
        ...
        internal static StringRebuilder StringRebuilderFromSnapshotSpans(IList<SnapshotSpan> sourceSpans,
            Span selectedSourceSpans)
        {
        ...
        {
            var textBuffer = new TextBuffer(contentType, content,
                TextDifferencingSelectorService.DefaultTextDifferencingService, GuardedOperations, spurnGroup);
