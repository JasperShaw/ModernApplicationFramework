...
namespace ModernApplicationFramework.Modules.Editor.Text
{
    ...
    {
        public static readonly NormalizedTextChangeCollection Empty =
            new NormalizedTextChangeCollection(new TextChange[0]);
        ...
        private NormalizedTextChangeCollection(IReadOnlyList<TextChange> changes,
            StringDifferenceOptions? differenceOptions, ITextDifferencingService textDifferencingService,
        ...
        {
            return GetTrivialCollection(changes) ?? new NormalizedTextChangeCollection(changes, differenceOptions,
                       textDifferencingService, before, after);
        }

        public static void Denormalize(INormalizedTextChangeCollection normalizedChanges,
            List<TextChange> denormChangesWithSentinel)
        {
            ...
            for (var index2 = 0; index2 < textChangeList.Count; ++index2)
            {
                ...
                while (PriorTo(denormChangesWithSentinel[index1], textChange1, accumulatedDelta,
                ...
                if (textChange1.OldEnd - accumulatedDelta > denormChangesWithSentinel[index1].OldPosition)
                {
                    ...
                    denormChangesWithSentinel.Insert(index1,
                        new TextChange(textChange1.OldPosition - accumulatedDelta,
                            TextChange.ChangeOldSubText(textChange1, 0, num1),
                            TextChange.NewStringRebuilder(textChange1), LineBreakBoundaryConditions.None));
                    accumulatedNormalizedDelta += num2;
                    var textChange2 = new TextChange(textChange1.OldPosition + num1,
                        TextChange.ChangeOldSubText(textChange1, num1, length), StringRebuilder.Empty,
                ...
                else
                {
                    denormChangesWithSentinel.Insert(index1,
                        new TextChange(textChange1.OldPosition - accumulatedDelta,
                            TextChange.OldStringRebuilder(textChange1), TextChange.NewStringRebuilder(textChange1),
                            LineBreakBoundaryConditions.None));
        ...
        private static LineBreakBoundaryConditions ComputeBoundaryConditions(TextChange outerChange,
            StringRebuilder oldText, Span leftSpan)
        {
            var boundaryConditions = LineBreakBoundaryConditions.None;
            if (leftSpan.Start == 0)
                boundaryConditions = outerChange.LineBreakBoundaryConditions &
                                     LineBreakBoundaryConditions.PrecedingReturn;
        ...
        private static IList<TextChange> GetChangesFromDifferenceCollection(ref int delta, TextChange originalChange,
            StringRebuilder oldText, StringRebuilder newText, IHierarchicalDifferenceCollection diffCollection,
            int leftOffset = 0, int rightOffset = 0)
        {
            ...
            for (var index = 0; index < diffCollection.Differences.Count; ++index)
            {
                ...
                var span2 = Translate(diffCollection.RightDecomposition.GetSpanInOriginal(difference.Right),
                    rightOffset);
                var containedDifferences = diffCollection.GetContainedDifferences(index);
                if (containedDifferences != null)
                {
                    textChangeList.AddRange(GetChangesFromDifferenceCollection(ref delta, originalChange, oldText,
                        newText, containedDifferences, span1.Start, span2.Start));
                }
                else
                {
        ...
        private static INormalizedTextChangeCollection GetTrivialCollection(IReadOnlyList<TextChange> changes)
        {
            ...
            if (changes.Count == 1)
            {
                ...
                if (change.OldLength + change.NewLength == 1 &&
                    change.LineBreakBoundaryConditions == LineBreakBoundaryConditions.None &&
                    change.LineCountDelta == 0)
                {
                    var isInsertion = change.NewLength == 1;
                    return new TrivialNormalizedTextChangeCollection(
                        isInsertion ? change.NewText[0] : change.OldText[0], isInsertion, change.OldPosition);
                }
            }
        ...
        private static IReadOnlyList<TextChange> Normalize(IReadOnlyList<TextChange> changes,
            StringDifferenceOptions? differenceOptions, ITextDifferencingService textDifferencingService,
            ITextSnapshot before, ITextSnapshot after)
        {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        if (num2 == 0)
                        {
                            ...
                            textChange1.LineBreakBoundaryConditions =
                                (textChange1.LineBreakBoundaryConditions &
                                 LineBreakBoundaryConditions.PrecedingReturn) |
                                (textChange2.LineBreakBoundaryConditions &
                                 LineBreakBoundaryConditions.SucceedingNewline);
                        }
                        else if (textChange1.OldEnd + num3 < textChange2.OldEnd)
                        {
                            var start = textChange1.OldEnd + num3 - textChange2.OldPosition;
                            stringRebuilder2 = stringRebuilder2.Append(
                                textChange2._oldText.GetSubText(Span.FromBounds(start, textChange2._oldText.Length)));
            ...
            if (differenceOptions.HasValue)
            {
                ...
                foreach (var originalChange in textChangeArray)
                    if (originalChange != null)
                    {
                        ...
                        else if (originalChange.OldLength >= TextModelOptions.DiffSizeThreshold ||
                                 originalChange.NewLength >= TextModelOptions.DiffSizeThreshold)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                diffCollection = textDifferencingService.DiffSnapshotSpans(
                                    new SnapshotSpan(before, originalChange.OldSpan),
                                    new SnapshotSpan(after, originalChange.NewSpan), differenceOptions1);
                            }
                            else
                            {
                                ...
                                if (oldText != newText)
                                    diffCollection =
                                        textDifferencingService.DiffStrings(oldText, newText, differenceOptions1);
                                else
                                    continue;
                            }

                            var delta = 0;
                            textChangeList.AddRange(GetChangesFromDifferenceCollection(ref delta, originalChange,
                                originalChange._oldText, originalChange._newText, diffCollection, 0, 0));
                        }
                    }
            }
            else
        ...
        private static bool PriorTo(ITextChange denormalizedChange, ITextChange normalizedChange, int accumulatedDelta,
            int accumulatedNormalizedDelta)
        {
            if (denormalizedChange.OldLength != 0 && normalizedChange.OldLength != 0)
                return denormalizedChange.OldPosition <=
                       normalizedChange.NewPosition - accumulatedDelta - accumulatedNormalizedDelta;
            return denormalizedChange.OldPosition <
                   normalizedChange.NewPosition - accumulatedDelta - accumulatedNormalizedDelta;
        }
