...
namespace ModernApplicationFramework.Modules.Editor.Text
{
    internal class HighFidelityTrackingPoint : TrackingPoint
    {
        ...
        internal HighFidelityTrackingPoint(ITextVersion version, int position, PointTrackingMode trackingMode,
        ...
        internal static int TrackPositionBackwardInTime(PointTrackingMode trackingMode,
            List<VersionNumberPosition> noninvertibleHistory, int currentPosition, ITextVersion currentVersion,
            ITextVersion targetVersion)
        {
            ...
            while (num > 0)
            {
                ...
                {
                    var index = noninvertibleHistory.BinarySearch(
                        new VersionNumberPosition(textVersion.VersionNumber, 0),
        ...
        internal static int TrackPositionForwardInTime(PointTrackingMode trackingMode, TrackingFidelityMode fidelity,
            ref List<VersionNumberPosition> noninvertibleHistory, int currentPosition, ITextVersion currentVersion,
            ITextVersion targetVersion)
        {
            ...
            while (textVersion != targetVersion)
            {
                ...
                if (fidelity == TrackingFidelityMode.UndoRedo &&
                    textVersion.ReiteratedVersionNumber != textVersion.VersionNumber && noninvertibleHistory != null)
                {
                    var index = noninvertibleHistory.BinarySearch(
                        new VersionNumberPosition(textVersion.ReiteratedVersionNumber, 0),
                        VersionNumberPositionComparer.Instance);
                    if (index >= 0)
                    {
                        currentPosition = noninvertibleHistory[index].Position;
                ...
                if (count == 0)
                {
                    if (textVersion.VersionNumber != textVersion.ReiteratedVersionNumber)
                        RecordNoninvertibleTransition(ref noninvertibleHistory, currentPosition,
                            textVersion.ReiteratedVersionNumber);
                }
                else
                {
                    ...
                    for (var index = 0; index < count; ++index)
                    {
                        ...
                        if (IsOpaque(change))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    RecordNoninvertibleTransition(ref noninvertibleHistory, currentPosition1,
        ...
        protected override int TrackPosition(ITextVersion targetVersion)
        {
            ...
            {
                position = TrackPositionForwardInTime(TrackingMode, _fidelity, ref noninvertibleHistory,
                    cachedPosition.Position, cachedPosition.Version, targetVersion);
                _cachedPosition = new VersionPositionHistory(targetVersion, position, noninvertibleHistory);
            }
            else
            {
                position = TrackPositionBackwardInTime(TrackingMode,
                    _fidelity == TrackingFidelityMode.Backward ? noninvertibleHistory : null, cachedPosition.Position,
        ...
        private static void RecordNoninvertibleTransition(ref List<VersionNumberPosition> noninvertibleHistory,
            int currentPosition, int versionNumber)
        {
            if (noninvertibleHistory == null)
                noninvertibleHistory = new List<VersionNumberPosition>();
            if (noninvertibleHistory.Count > 0 &&
                noninvertibleHistory[noninvertibleHistory.Count - 1].VersionNumber == versionNumber)
                return;
        ...
        private class VersionPositionHistory
        {
            ...
            public VersionPositionHistory(ITextVersion version, int position,
