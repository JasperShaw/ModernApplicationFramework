...
namespace ModernApplicationFramework.Modules.Editor.Text
{
    internal class StringRebuilderForString : UnaryStringRebuilder
    {
        ...
        private StringRebuilderForString(string source, ILineBreaks lineBreaks, int start, int lenght,
            int lineBreaksStart,
            ...
            int lineBreaksStart, int lineBreaksLength)
        {
            if (lineBreaksLength == 0)
                return new StringRebuilderForString(source, LineBreakManager.Empty, start, length, 0, 0, source[start],
