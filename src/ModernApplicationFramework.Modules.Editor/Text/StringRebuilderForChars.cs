...
namespace ModernApplicationFramework.Modules.Editor.Text
{
    internal class StringRebuilderForChars : UnaryStringRebuilder
    {
        ...
        private StringRebuilderForChars(char[] source, ILineBreaks lineBreaks, int start, int length,
        ...
        public override StringRebuilder GetSubText(Span span)
        {
            ...
            return Create(Content, LineBreaks, span.Start + TextSpanStart, span.Length, firstLineNumber,
        ...
        internal static StringRebuilder Create(char[] source, ILineBreaks lineBreaks, int start, int length,
            int lineBreaksStart, int lineBreaksLength)
        {
            if (lineBreaksLength == 0)
                return new StringRebuilderForChars(source, LineBreakManager.Empty, start, length, 0, 0, source[start],
                    source[start + length - 1]);
            return new StringRebuilderForChars(source, lineBreaks, start, length, lineBreaksStart, lineBreaksLength,
                source[start], source[start + length - 1]);
        }
    }
}