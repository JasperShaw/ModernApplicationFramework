...
namespace ModernApplicationFramework.Modules.Editor.EditorPrimitives
{
    internal sealed class DefaultTextPointPrimitive : TextPoint
    {
        ...
        public override SnapshotPoint AdvancedTextPoint =>
            _trackingPoint.GetPoint(TextBuffer.AdvancedTextBuffer.CurrentSnapshot);
        ...
        public override int Column
        {
            get
            {
                ...
                var stringInfo =
                    new StringInfo(
                        TextBuffer.AdvancedTextBuffer.CurrentSnapshot.GetText(Span.FromBounds(containingLine.Start,
                            point.Position)));
                var num = 0;
                for (var startingTextElement = 0;
                    startingTextElement < stringInfo.LengthInTextElements;
                    ++startingTextElement)
        ...
        public override int CurrentPosition =>
            _trackingPoint.GetPosition(TextBuffer.AdvancedTextBuffer.CurrentSnapshot);

        public override int EndOfLine => _trackingPoint.GetPoint(TextBuffer.AdvancedTextBuffer.CurrentSnapshot)
            .GetContainingLine().End;

        public override int LineNumber => _trackingPoint.GetPoint(TextBuffer.AdvancedTextBuffer.CurrentSnapshot)
            .GetContainingLine().LineNumber;

        public override int StartOfLine => _trackingPoint.GetPoint(TextBuffer.AdvancedTextBuffer.CurrentSnapshot)
            .GetContainingLine().Start;

        public override PrimitiveTextBuffer TextBuffer { get; }

        public DefaultTextPointPrimitive(PrimitiveTextBuffer textBuffer, int position, ITextSearchService findLogic,
            IEditorOptions editorOptions, ITextStructureNavigator textStructureNavigator,
            IBufferPrimitivesFactoryService bufferPrimitivesFactory)
        {
            ...
            _trackingPoint =
                TextBuffer.AdvancedTextBuffer.CurrentSnapshot.CreateTrackingPoint(0, PointTrackingMode.Positive);
        ...
        {
            return PrimitivesUtilities.Delete(TextBuffer.AdvancedTextBuffer, CurrentPosition,
                GetNextCharacter().Length);
        }
        ...
        public override Collection<TextRange> FindAll(string pattern, FindOptions findOptions)
        {
            ...
            {
                var textRange = _bufferPrimitivesFactory.CreateTextRange(TextBuffer,
                    _bufferPrimitivesFactory.CreateTextPoint(TextBuffer, snapshotSpan.Start),
                    _bufferPrimitivesFactory.CreateTextPoint(TextBuffer, snapshotSpan.End));
                ...
                if (textExtent1.Span.Start == CurrentPosition)
                {
                    if (!char.IsWhiteSpace(TextBuffer.AdvancedTextBuffer.CurrentSnapshot[textExtent1.Span.Start - 1]))
                        textExtent1 = GetTextExtent(textExtent1.Span.Start - 1);
                         ...
                         !char.IsWhiteSpace(TextBuffer.AdvancedTextBuffer.CurrentSnapshot[textExtent1.Span.Start - 1]))
                {
                    textExtent1 =
                        new TextExtent(new SnapshotSpan(
                            new TextExtent(GetTextExtent(textExtent1.Span.Start - 1)).Span.Start,
        ...
        public override string GetNextCharacter()
        {
            ...
            {
                var str2 = str1 + currentSnapshot[index];
        ...
        public override TextRange GetNextWord()
        {
            ...
            if (textExtent1.Span.End >= TextBuffer.AdvancedTextBuffer.CurrentSnapshot.Length)
                return _bufferPrimitivesFactory.CreateTextRange(TextBuffer, TextBuffer.GetEndPoint(),
                    TextBuffer.GetEndPoint());
        ...
        public override string GetPreviousCharacter()
        {
            ...
            while (--index >= 0)
            {
                var str2 = currentSnapshot[index] + str1;
                if (StringInfo.GetNextTextElement(str2).Length > str1.Length)
                    str1 = str2;
                else
                    break;
        ...
        public override TextRange GetPreviousWord()
        {
            ...
            if (currentWord1.GetStartPoint().CurrentPosition <= 0)
                return _bufferPrimitivesFactory.CreateTextRange(TextBuffer, TextBuffer.GetStartPoint(),
                    TextBuffer.GetStartPoint());
        ...
        {
            var tabSize = _editorOptions.GetTabSize();
            return InsertText(_editorOptions.IsConvertTabsToSpacesEnabled()
                ? new string(' ', tabSize - Column % tabSize)
                : "\t");
        }
        ...
        public override bool InsertText(string text)
        {
            ...
            if (text.Length > 0)
                return PrimitivesUtilities.Insert(TextBuffer.AdvancedTextBuffer,
                    _trackingPoint.GetPosition(TextBuffer.AdvancedTextBuffer.CurrentSnapshot), text);
            return true;
        }
        ...
        public override void MoveTo(int position)
        {
            ...
            _trackingPoint =
                TextBuffer.AdvancedTextBuffer.CurrentSnapshot.CreateTrackingPoint(position, PointTrackingMode.Positive);
        }
        ...
        public override void MoveToBeginningOfPreviousLine()
        {
            ...
            if (containingLine.LineNumber != 0)
                MoveTo(TextBuffer.AdvancedTextBuffer.CurrentSnapshot
                    .GetLineFromLineNumber(containingLine.LineNumber - 1).Start);
            else
                MoveToStartOfLine();
        }
        ...
        public override bool TransposeCharacter()
        {
            ...
            return PrimitivesUtilities.Replace(TextBuffer.AdvancedTextBuffer, new Span(span1.Start, replacement.Length),
                replacement);
        }
        ...
        public override bool TransposeLine()
        {
            ...
            return TransposeLine((lineFromPosition.LineNumber != currentSnapshot.LineCount - 1
                ? currentSnapshot.GetLineFromLineNumber(lineFromPosition.LineNumber + 1)
                : currentSnapshot.GetLineFromLineNumber(lineFromPosition.LineNumber - 1)).LineNumber);
        }
        ...
        public override bool TransposeLine(int lineNumber)
        {
            ...
            {
                if (edit.Replace(lineFromPosition.Extent, lineFromLineNumber.GetText()) &&
                    edit.Replace(lineFromLineNumber.Extent, lineFromPosition.GetText()))
                {
                    edit.Apply();
                    if (edit.Canceled)
                        return false;
        ...
        {
            return new DefaultTextPointPrimitive(TextBuffer, CurrentPosition, _findLogic, _editorOptions,
                _textStructureNavigator, _bufferPrimitivesFactory);
        }
        ...
        private static int FindStartOfCombiningSequence(ITextSnapshot snapshot, int position)
        {
            ...
            if (position == snapshot.Length ||
                !IsPotentialCombiningCharacter(CharUnicodeInfo.GetUnicodeCategory(snapshot[position])))
                return position;
            ...
            for (--position; position > start; --position)
                ...
                {
                    var combiningCharacters =
                        StringInfo.ParseCombiningCharacters(snapshot.GetText(Span.FromBounds(position, end)));
        ...
        {
            if (category != UnicodeCategory.EnclosingMark && category != UnicodeCategory.NonSpacingMark &&
        ...
        private TextRange FindMatch(string pattern, FindOptions findOptions, TextPoint endPoint)
        {
            ...
            var next = _findLogic.FindNext(
                (findData.FindOptions & FindOptions.SearchReverse) != FindOptions.SearchReverse || wraparound
                    ? CurrentPosition
                    : endPoint.CurrentPosition, wraparound, findData);
            if (next.HasValue)
            {
                ...
                var textBuffer1 = TextBuffer;
                var primitivesFactory2 = _bufferPrimitivesFactory;
                var textBuffer2 = TextBuffer;
                snapshotSpan = next.Value;
        ...
        {
            return _textStructureNavigator.GetExtentOfWord(
                new SnapshotPoint(TextBuffer.AdvancedTextBuffer.CurrentSnapshot, position));
        }
