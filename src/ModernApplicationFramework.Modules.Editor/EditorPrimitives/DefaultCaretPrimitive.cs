...
namespace ModernApplicationFramework.Modules.Editor.EditorPrimitives
{
    internal sealed class DefaultCaretPrimitive : Caret
    {
        ...
        public override int DisplayColumn
        {
            get
            {
                ...
                if (textViewLine != null && textViewLine.IsValid)
                    return PrimitivesUtilities.GetColumnOfPoint(TextView.AdvancedTextView.TextSnapshot,
                        AdvancedTextPoint, textViewLine.Start, _editorOptions.GetTabSize(),
        ...
        private DisplayTextPoint CaretPoint =>
            TextView.GetTextPoint(TextView.AdvancedTextView.Caret.Position.BufferPosition);
        ...
        public override void MoveToEndOfViewLine(bool extendSelection)
        {
            ...
            {
                AdvancedCaret.MoveTo(TextView.AdvancedTextView.GetTextViewLineContainingBufferPosition(position).End,
                    PositionAffinity.Predecessor);
            }
            else
            {
                var containingTextViewLine = AdvancedCaret.ContainingTextViewLine;
                AdvancedCaret.MoveTo(containingTextViewLine.End,
                    containingTextViewLine.IsLastTextViewLineForSnapshotLine
        ...
        public override void MoveToLine(int lineNumber, int offset, bool extendSelection)
        {
            ...
            if (!extendSelection)
                TextView.AdvancedTextView.Selection.Clear();
            AdvancedCaret.MoveTo(
                new VirtualSnapshotPoint(TextView.AdvancedTextView.TextSnapshot.GetLineFromLineNumber(lineNumber),
                    offset));
            AdvancedCaret.EnsureVisible();
            UpdateSelection(extendSelection, virtualBufferPosition);
        }
        ...
        public override void MoveToNextLine(bool extendSelection)
        {
            ...
            if (!textLine.IsLastTextViewLineForSnapshotLine || textLine.LineBreakLength != 0)
                textLine =
                    TextView.AdvancedTextView.GetTextViewLineContainingBufferPosition(textLine.EndIncludingLineBreak);
            if (!extendSelection)
                TextView.AdvancedTextView.Selection.Clear();
        ...
        {
            var virtualBufferPosition = AdvancedCaret.Position.VirtualBufferPosition;
            var textLine = !(TextView.Selection.IsEmpty | extendSelection)
                ? TextView.AdvancedTextView.GetTextViewLineContainingBufferPosition(TextView.AdvancedTextView.Selection
                    .Start.Position)
                : TextView.AdvancedTextView.Caret.ContainingTextViewLine;
            if (textLine.Start != 0)
        ...
        public override void MoveToStartOfViewLine(bool extendSelection)
        {
            ...
            if (!extendSelection)
                TextView.AdvancedTextView.Selection.Clear();
            AdvancedCaret.MoveTo(
                (num != 0 || isReversed || extendSelection
                    ? AdvancedCaret.ContainingTextViewLine
                    : TextView.AdvancedTextView.GetTextViewLineContainingBufferPosition(position)).Start,
                PositionAffinity.Successor);
            AdvancedCaret.EnsureVisible();
            UpdateSelection(extendSelection, virtualBufferPosition);
        }
        ...
        private void PageUpDown(ScrollDirection direction)
        {
            if (direction == ScrollDirection.Up)
            {
                ...
                var bufferPosition = firstVisibleLine.VisibilityState == VisibilityState.FullyVisible
                    ? firstVisibleLine.Start
                    : firstVisibleLine.EndIncludingLineBreak;
                if (TextView.AdvancedTextView.ViewScroller.ScrollViewportVerticallyByPage(direction))
                {
                    if (TextView.AdvancedTextView.ViewportBottom > TextView.AdvancedTextView.TextViewLines
                            .GetTextViewLineContainingBufferPosition(bufferPosition).Bottom)
            ...
            else
            {
                ...
                var bufferPosition =
                    lastVisibleLine.VisibilityState == VisibilityState.FullyVisible || (int) lastVisibleLine.Start == 0
                        ? lastVisibleLine.Start
                        : lastVisibleLine.Start - 1;
                if (TextView.AdvancedTextView.ViewScroller.ScrollViewportVerticallyByPage(direction))
                {
                    if (TextView.AdvancedTextView.TextViewLines.GetTextViewLineContainingBufferPosition(bufferPosition)
                            .Bottom > TextView.AdvancedTextView.ViewportTop)
