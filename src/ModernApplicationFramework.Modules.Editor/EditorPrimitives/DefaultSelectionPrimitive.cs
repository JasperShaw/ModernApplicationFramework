    ...
    internal sealed class DefaultSelectionPrimitive : Selection
    {
        ...
        public override bool Capitalize()
        {
            ...
            if (!TextRange.Capitalize())
                return false;
            if (!isEmpty)
                TextSelection.Select(new SnapshotSpan(TextView.AdvancedTextView.TextSnapshot, advancedTextRange),
                    isReversed);
            return true;
        }
        ...
        public override void ExtendSelection(TextPoint newEnd)
        {
            SelectRange(
                !IsEmpty
                    ? (!IsReversed ? GetStartPoint().CurrentPosition : GetEndPoint().CurrentPosition)
                    : TextSelection.Start.Position, newEnd.CurrentPosition);
        }
        ...
        public override bool Indent()
        {
            ...
            else
            {
                ...
                var flag1 = start1.Position <= TextView.GetTextPoint(lineFromPosition1.Start)
                                .GetFirstNonWhiteSpaceCharacterOnLine().CurrentPosition;
                var flag2 = end1.Position < TextView.GetTextPoint(lineFromPosition2.Start)
                                .GetFirstNonWhiteSpaceCharacterOnLine().CurrentPosition;
        ...
        private void SelectRange(int selectionStart, int selectionEnd)
        {
            ...
            var containingBufferPosition =
                TextView.AdvancedTextView.GetTextViewLineContainingBufferPosition(snapshotPoint);
            var caretAffinity =
                containingBufferPosition.IsLastTextViewLineForSnapshotLine ||
                snapshotPoint != containingBufferPosition.End
            ...
            TextView.AdvancedTextView.ViewScroller.EnsureSpanVisible(TextSelection.StreamSelectionSpan.SnapshotSpan,
                selectionStart <= selectionEnd
                    ...
                    : EnsureSpanVisibleOptions.ShowStart | EnsureSpanVisibleOptions.MinimumScroll);
        }
    }
}