...
namespace ModernApplicationFramework.Modules.Editor.Differencing
{
    internal class WrappedLineDifferenceCollection : IHierarchicalDifferenceCollection
    {
        ...
        public WrappedLineDifferenceCollection(IDifferenceCollection<string> original, IList<int> notIgnoredDifferences,
            ITextSnapshot leftSnapshot, ITextSnapshot rightSnapshot, ITokenizedStringList leftDecomposition,
            ITokenizedStringList rightDecomposition, ITextDifferencingService diffService,
            StringDifferenceOptions nextLevelOptions)
        {
            _original = original.Differences.Count != notIgnoredDifferences.Count
                ? new DifferenceCollection<string>(CreateDiffs(original, notIgnoredDifferences), original.LeftSequence,
                    original.RightSequence)
                : original;
        ...
        internal IHierarchicalDifferenceCollection CalculateContainedDiff(int index)
        {
            ...
            if (difference.DifferenceType == DifferenceType.Change)
            {
                ...
                {
                    var wordDifferences = _diffService.DiffSnapshotSpans(
                        new SnapshotSpan(_leftSnapshot, spanInOriginal1),
                        new SnapshotSpan(_rightSnapshot, spanInOriginal2), _nextLevelOptions);
        ...
        private static IList<Difference> CreateDiffs(IDifferenceCollection<string> original,
            IList<int> notIgnoredDifferences)
        {
            ...
            if (notIgnoredDifferences.Count != 0)
            {
                ...
                for (var index = 1; index < notIgnoredDifferences.Count; ++index)
                {
                    ...
                    DifferenceCollection<string>.AddDifference(start1, end1, start2, start3, end2, start4, diffs,
                        ref local);
                ...
                DifferenceCollection<string>.AddDifference(start5, end3, count1, start6, end4, count2, diffs1,
                    ref local1);
            }
