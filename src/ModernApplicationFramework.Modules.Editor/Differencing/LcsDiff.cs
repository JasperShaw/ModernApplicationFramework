...
namespace ModernApplicationFramework.Modules.Editor.Differencing
{
    internal class LcsDiff<T> : DiffFinder<T>
    {
        ...
        protected override IDiffChange[] ComputeDiff(int originalStart, int originalEnd, int modifiedStart,
            int modifiedEnd)
        {
        ...
        private IDiffChange[] ComputeDiffRecursive(int originalStart, int originalEnd, int modifiedStart,
            int modifiedEnd, out bool quitEarly)
        {
            quitEarly = false;
            for (;
            ...
            var recursionPoint = ComputeRecursionPoint(originalStart, originalEnd, modifiedStart, modifiedEnd,
                out var midOriginal, out var midModified, out quitEarly);
            if (recursionPoint != null)
                return recursionPoint;
            if (!quitEarly)
            {
                var diffRecursive = ComputeDiffRecursive(originalStart, midOriginal, modifiedStart, midModified,
                    out quitEarly);
                IDiffChange[] right;
                if (!quitEarly)
                    right = ComputeDiffRecursive(midOriginal + 1, originalEnd, midModified + 1, modifiedEnd,
                        out quitEarly);
                else
                    right = new IDiffChange[]
                    {
                        new DiffChange(midOriginal + 1, originalEnd - (midOriginal + 1) + 1, midModified + 1,
            ...
            {
                new DiffChange(originalStart, originalEnd - originalStart + 1, modifiedStart,
                    modifiedEnd - modifiedStart + 1)
            };
        }

        private IDiffChange[] ComputeRecursionPoint(int originalStart, int originalEnd, int modifiedStart,
            int modifiedEnd, out int midOriginal, out int midModified, out bool quitEarly)
        {
            var sourceIndex1 = 0;
            var num1 = 0;
            var sourceIndex2 = 0;
            ...
            for (var numDifferences = 1; numDifferences <= num3 / 2 + 1; ++numDifferences)
            {
                ...
                var num6 = 0;
                sourceIndex1 = ClipDiagonalBound(diagonalBaseIndex1 - numDifferences, numDifferences,
                    diagonalBaseIndex1, numDiagonals);
                num1 = ClipDiagonalBound(diagonalBaseIndex1 + numDifferences, numDifferences, diagonalBaseIndex1,
                    numDiagonals);
                var index1 = sourceIndex1;
                while (index1 <= num1)
                {
                    var num7 = index1 == sourceIndex1 || index1 < num1 && numArray1[index1 - 1] < numArray1[index1 + 1]
                        ? numArray1[index1 + 1]
                        : numArray1[index1 - 1] + 1;
                    ...
                    if (!flag && Math.Abs(index1 - diagonalBaseIndex2) <= numDifferences - 1 &&
                        num7 >= numArray2[index1])
                    {
                        midOriginal = num7;
                        midModified = num8;
                ...
                if (ContinueDifferencePredicate != null &&
                    !ContinueDifferencePredicate(originalIndex1, OriginalSequence, longestMatchSoFar))
                {
                    quitEarly = true;
                ...
                sourceIndex2 = ClipDiagonalBound(diagonalBaseIndex2 - numDifferences, numDifferences,
                    diagonalBaseIndex2, numDiagonals);
                num2 = ClipDiagonalBound(diagonalBaseIndex2 + numDifferences, numDifferences, diagonalBaseIndex2,
                    numDiagonals);
                var index2 = sourceIndex2;
                while (index2 <= num2)
                {
                    var originalIndex2 =
                        index2 == sourceIndex2 || index2 < num2 && numArray2[index2 - 1] >= numArray2[index2 + 1]
                            ? numArray2[index2 + 1] - 1
                            : numArray2[index2 - 1];
                    var modifiedIndex = originalIndex2 - (index2 - diagonalBaseIndex2) - num5;
                    var num7 = originalIndex2;
                    for (;
                        originalIndex2 > originalStart && modifiedIndex > modifiedStart &&
                        ElementsAreEqual(originalIndex2, modifiedIndex);
                        --modifiedIndex)
                        --originalIndex2;
                    numArray2[index2] = originalIndex2;
                    if (flag && Math.Abs(index2 - diagonalBaseIndex1) <= numDifferences &&
                        originalIndex2 <= numArray1[index2])
                    {
        ...
        private IDiffChange[] ShiftChanges(IDiffChange[] changes)
        {
            for (var index = 0; index < changes.Length; ++index)
            {
                ...
                for (var flag2 = change.ModifiedLength > 0;
                    change.OriginalStart + change.OriginalLength < num1 &&
                    change.ModifiedStart + change.ModifiedLength < num2 &&
                    (!flag1 || OriginalElementsAreEqual(change.OriginalStart,
                         change.OriginalStart + change.OriginalLength)) &&
                    (!flag2 || ModifiedElementsAreEqual(change.ModifiedStart,
            ...
            for (var index = 0; index < changes.Length; ++index)
                if (index < changes.Length - 1 &&
                    ChangesOverlap(changes[index], changes[index + 1], out var mergedChange))
                {
