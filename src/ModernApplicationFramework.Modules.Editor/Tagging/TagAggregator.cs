...
namespace ModernApplicationFramework.Modules.Editor.Tagging
{
    internal sealed class TagAggregator<T> : IAccurateTagAggregator<T> where T : ITag
    {
        ...
        public TagAggregator(TagAggregatorFactoryService factory, ITextView textView, IBufferGraph bufferGraph,
        ...
        public IEnumerable<IMappingTagSpan<T>> GetAllTags(NormalizedSnapshotSpanCollection snapshotSpans,
        ...
        internal IList<ITagger<T>> GatherTaggers(ITextBuffer textBuffer)
        {
            ...
            foreach (var eligibleFactory in TagAggregatorFactoryService.GuardedOperations.FindEligibleFactories(
                TagAggregatorFactoryService.GetBufferTaggersForType(textBuffer.ContentType, typeof(T)),
                textBuffer.ContentType, TagAggregatorFactoryService.ContentTypeRegistryService))
            {
                ...
                {
                    TagAggregatorFactoryService.GuardedOperations.HandleException(
                        taggerProvider ?? (object) eligibleFactory, ex);
                }
            ...
            if (TextView != null)
                foreach (var eligibleFactory in TagAggregatorFactoryService.GuardedOperations.FindEligibleFactories(
                    TagAggregatorFactoryService.GetViewTaggersForType(textBuffer.ContentType, typeof(T)).Where(f =>
                    {
                    ...
                    }), textBuffer.ContentType, TagAggregatorFactoryService.ContentTypeRegistryService))
                {
                    ...
                    {
                        TagAggregatorFactoryService.GuardedOperations.HandleException(
                            viewTaggerProvider ?? (object) eligibleFactory, ex);
                    }
        ...
        private void BufferGraph_GraphBufferContentTypeChanged(object sender, GraphBufferContentTypeChangedEventArgs e)
        {
            if (Disposed || !Initialized || _options.HasFlag(TagAggregatorOptions.NoProjection) &&
                e.TextBuffer != BufferGraph.TopBuffer)
                return;
            ...
            RaiseEvents(this,
                BufferGraph.CreateMappingSpan(new SnapshotSpan(currentSnapshot, 0, currentSnapshot.Length),
        ...
        private IEnumerable<IMappingTagSpan<T>> GetTagsForBuffer(
            KeyValuePair<ITextBuffer, IList<ITagger<T>>> bufferAndTaggers,
            NormalizedSnapshotSpanCollection snapshotSpans, ITextSnapshot root, CancellationToken? cancel)
        {
            ...
            foreach (var t1 in bufferAndTaggers.Value)
            {
                ...
                try
                {
                    ...
                    if (cancel.HasValue)
                    {
                        ...
                        tagSpans = !(tagger is IAccurateTagger<T> accurateTagger)
                            ? tagger.GetTags(snapshotSpans)
                            : accurateTagger.GetAllTags(snapshotSpans, cancel.Value);
                    }
                    else
                    {
                        ...
                        while (true)
                        {
                            ...
                            do
                            {
                                ...
                                if (tagSpan != null)
                                    ...
                                    goto label_19;
                            } while (!snapshotSpans.IntersectsWith(span.TranslateTo(snapshot,
                                SpanTrackingMode.EdgeExclusive)));

                            yield return new MappingTagSpan<T>(
                                root == null
                                    ? BufferGraph.CreateMappingSpan(span, SpanTrackingMode.EdgeExclusive)
                                    : MappingSpanSnapshot.Create(root, span, SpanTrackingMode.EdgeExclusive,
                                        BufferGraph), tagSpan.Tag);
                        }
                    }
                    finally
                    {
        ...
        private IEnumerable<IMappingTagSpan<T>> InternalGetTags(NormalizedSnapshotSpanCollection snapshotSpans,
            CancellationToken? cancel)
        {
        ...
        private void RaiseBatchedTagsChanged()
        {
            ...
            TagAggregatorFactoryService.GuardedOperations.RaiseEvent(this, batchedTagsChanged,
                new BatchedTagsChangedEventArgs(mappingSpanList));
        }
        ...
        private void RaiseEvents(object sender, IMappingSpan span)
        {
            ...
            while (true)
            {
                var mappingSpanLink2 = Interlocked.CompareExchange(ref AcculumatedSpanLinks,
                    new MappingSpanLink(mappingSpanLink1, span), mappingSpanLink1);
